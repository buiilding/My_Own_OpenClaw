...
describe('renderer chat runtime boundary', () => {
  ...
  test('display-row chat projection consumes SDK source event metadata', async () => {
    ...
    expect(projectionRuntimeSource).toContain('isUserDisplayRow(row) && actionRecord.canEdit');
    expect(projectionRuntimeSource).toContain('isAssistantDisplayRow(row) && actionRecord.canRetry');
