...
} = require('../../frontend/src/main/ipc/ipc_direct_wake_up_agent_adapter_deps.cjs');
...
describe('ipc direct wake-up agent adapter deps runtime', () => {
  ...
  test('ipc.cjs delegates direct wake-up adapter dependency construction to the helper', async () => {
    ...
    const helperSource = await fs.readFile(
      path.resolve(__dirname, '../../frontend/src/main/ipc/ipc_direct_wake_up_agent_adapter_deps.cjs'),
      'utf8',
    );
