...
} = require('../../frontend/src/main/ipc/ipc_process_reset_runtime.cjs');
...
describe('ipc process reset runtime', () => {
  ...
  test('shutdown reset preserves process cleanup order', () => {
    ...
    const runtime = createIpcProcessResetRuntime({
      ...
      installAuthIdentityRuntime: resettable('installAuthIdentityRuntime', calls),
      activeQueryContextState: resettable('activeQueryContextState', calls),
      desktopUiConfigCache: resettable('desktopUiConfigCache', calls),
  ...
  test('ipc.cjs delegates reset orchestration to the reset runtime', async () => {
    ...
    const runtimeSource = await fs.readFile(
      path.resolve(__dirname, '../../frontend/src/main/ipc/ipc_process_reset_runtime.cjs'),
      'utf8',
    );
