...
class TestBrowserControllerBasics:
    ...
    @mock.patch("tools.browser.controller.is_cdp_available")
    @mock.patch("tools.browser.controller.ensure_chrome_with_cdp")
    @mock.patch("tools.browser.controller.async_playwright")
    async def test_auto_connect_to_chrome_uses_dedicated_cdp_endpoint(
        self, mock_playwright, mock_ensure_chrome, mock_is_cdp_available
    ):
        """Connect to the Windie-dedicated Chrome CDP endpoint."""
        mock_is_cdp_available.return_value = True
        mock_ensure_chrome.return_value = "http://127.0.0.1:9333"
        ...
        controller = BrowserController()
        result = await controller.auto_connect_to_chrome("http://127.0.0.1:9333")

        assert result["status"] == "connected"
        assert result["auto_launched"] is False
        mock_pw.chromium.connect_over_cdp.assert_awaited_once_with(
            "http://127.0.0.1:9333",
            timeout=30000,
        )
...
class TestBrowserControllerActions:
    ...
    def _setup_select_locator(
        ...
        *,
        ...
        if select_option_side_effect is not None:
            mock_locator.select_option = mock.AsyncMock(
                side_effect=select_option_side_effect
            )
        else:
            mock_locator.select_option = mock.AsyncMock(
                return_value=select_option_return_value
            )
    ...
    async def test_click_option_prefers_select_option_fallback(self):
        ...
        mock_locator = mock.MagicMock()
        mock_locator.click = mock.AsyncMock(
            side_effect=Exception("intercepts pointer events")
        )
        mock_locator.dblclick = mock.AsyncMock()
        mock_locator.evaluate = mock.AsyncMock(
            return_value={
    ...
    async def test_click_role_ref_prefers_visible_in_viewport_candidate(self):
        ...
        offscreen.bounding_box = mock.AsyncMock(
            return_value={"x": 0, "y": 1800, "width": 120, "height": 24}
        )
        ...
        onscreen.bounding_box = mock.AsyncMock(
            return_value={"x": 10, "y": 120, "width": 120, "height": 24}
        )
    ...
    @pytest.mark.asyncio
    async def test_click_role_ref_returns_ambiguity_error_when_multiple_in_viewport(
        self,
    ):
        ...
        first.bounding_box = mock.AsyncMock(
            return_value={"x": 10, "y": 120, "width": 120, "height": 24}
        )
        ...
        second.bounding_box = mock.AsyncMock(
            return_value={"x": 20, "y": 180, "width": 120, "height": 24}
        )
    ...
    @pytest.mark.asyncio
    async def test_get_status_switches_to_remaining_live_tab_when_active_page_is_closed(
        self,
    ):
        """Closed active pages should not break status polling."""
        closed_page = mock.MagicMock()
        closed_page.is_closed.return_value = True
        closed_page.title = mock.AsyncMock(
            side_effect=AssertionError("closed page title should not be read")
        )
...
class TestBrowserControllerSnapshot:
    ...
    @pytest.mark.asyncio
    async def test_get_ai_snapshot_falls_back_to_dom_query_when_enhanced_path_fails(
        self,
    ):
    ...
    async def test_get_aria_snapshot(self):
        ...
        mock_locator = mock.MagicMock()
        mock_locator.aria_snapshot = mock.AsyncMock(return_value='- button "Submit"')
    ...
    async def test_get_aria_snapshot_truncates_to_max_chars(self):
        ...
        snapshot = await self.controller.get_page_snapshot(
            format_type="aria", max_chars=200
        )
