"""
Test file for glob tool that simulates the full LLM tool call pipeline.

This test simulates what happens when the LLM generates a correct tool call format:
{"functionCall": {"name": "glob", "args": {"pattern": "tests/**/*.py"}}}
...
async def test_glob_tool_pipeline():
    """
    Test the complete pipeline for a glob tool call.

    Simulates what happens when the LLM outputs:
    {"functionCall": {"name": "glob", "args": {"pattern": "tests/**/*.py"}}}
    """
    print("=" * 80)
    print("TESTING GLOB TOOL PIPELINE")
    print("=" * 80)

    # Step 1: Simulate LLM Response
    print("\n🔧 STEP 1: Simulating LLM Response")
    llm_response = '{"functionCall": {"name": "glob", "args": {"pattern": "tests/**/*.py"}}}'

    print(f"LLM Response: {llm_response}")

    # Step 2: Parse the Response
    print("\n📋 STEP 2: Parsing LLM Response")
    parser = create_test_parser(tool_names=["glob"])
...
if __name__ == "__main__":
    # Run the test
    asyncio.run(test_glob_tool_pipeline())
