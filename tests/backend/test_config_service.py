...
class TestConfigurationService:
    ...
    def service(self, mock_config_manager, mock_event_bus):
        return ConfigurationService(
            config_manager=mock_config_manager, event_bus=mock_event_bus
        )
    ...
    @pytest.mark.asyncio
    async def test_update_config_serializes_concurrent_updates(
        self, service, mock_config_manager
    ):
        ...
        manager_calls = []
        manager_calls_lock = threading.Lock()

        def update_with_controlled_overlap(new_cfg: AppConfig) -> AppConfig:
            with manager_calls_lock:
                manager_calls.append(new_cfg.model_provider)
            if new_cfg.model_provider == "anthropic":
                first_update_entered.set()
                assert release_first_update.wait(timeout=2.0)
            return new_cfg

        mock_config_manager.update_config.side_effect = update_with_controlled_overlap

        task_one = asyncio.create_task(service.update_config(first_config))
        assert await asyncio.to_thread(first_update_entered.wait, 2.0)
        task_two = asyncio.create_task(service.update_config(second_config))
        await asyncio.sleep(0.05)

        with manager_calls_lock:
            assert manager_calls == ["anthropic"]
    ...
    async def test_reload_config_success(
        ...
        assert event.new_config == reloaded_config
