"""
Test file for search_file_content tool that simulates the full LLM tool call pipeline.

This test simulates what happens when the LLM generates a correct tool call format:
{"functionCall": {"name": "search_file_content", "args": {"pattern": "def test_", "path": "tests/backend"}}}
...
async def test_search_file_content_tool_pipeline():
    """
    Test the complete pipeline for a search_file_content tool call.

    Simulates what happens when the LLM outputs:
    {"functionCall": {"name": "search_file_content", "args": {"pattern": "def test_", "path": "tests/backend"}}}
    """
    print("=" * 80)
    print("TESTING SEARCH FILE CONTENT TOOL PIPELINE")
    print("=" * 80)

    # Step 1: Simulate LLM Response
    print("\n🔧 STEP 1: Simulating LLM Response")
    llm_response = '{"functionCall": {"name": "search_file_content", "args": {"pattern": "def test_", "path": "tests/backend"}}}'

    print(f"LLM Response: {llm_response}")

    # Step 2: Parse the Response
    print("\n📋 STEP 2: Parsing LLM Response")
    parser = create_test_parser(tool_names=["search_file_content"])
...
if __name__ == "__main__":
    # Run the test
    asyncio.run(test_search_file_content_tool_pipeline())
