"""
Test file for get_open_windows tool that simulates the full LLM tool call pipeline.

This test simulates what happens when the LLM generates a correct tool call format:
{"functionCall": {"name": "get_open_windows", "args": {"explanation": "Checking what applications are currently running"}}}
...
async def test_get_open_windows_tool_pipeline():
    """
    Test the complete pipeline for a get_open_windows tool call.

    Simulates what happens when the LLM outputs:
    {"functionCall": {"name": "get_open_windows", "args": {"explanation": "Checking what applications are currently running"}}}
    """
    print("=" * 80)
    print("TESTING GET_OPEN_WINDOWS TOOL PIPELINE")
    print("=" * 80)

    # Step 1: Simulate LLM Response
    print("\n🔧 STEP 1: Simulating LLM Response")
    llm_response = '{"functionCall": {"name": "get_open_windows", "args": {"explanation": "Checking what applications are currently running to avoid launching duplicates"}}}'

    print(f"LLM Response: {llm_response}")

    # Step 2: Parse the Response
    print("\n📋 STEP 2: Parsing LLM Response")
    parser = create_test_parser(tool_names=["get_open_windows"])
...
if __name__ == "__main__":
    # Run the test
    asyncio.run(test_get_open_windows_tool_pipeline())
