...
class TestEventHandlerWrapper:
    ...
    def test_init_with_bound_method(self):
        ...
        assert wrapper.handler_key == (
            "method",
            id(original_handler.__self__),
            id(original_handler.__func__),
        )
...
class TestEventBus:
    ...
    def test_subscribe(self, bus):
        ...
        assert MockEvent in bus._store._subscribers
        assert len(bus._store._subscribers[MockEvent]) == 1
        assert bus._store._subscribers[MockEvent][0].handler is handler
    ...
    def test_subscribe_with_priority(self, bus):
        ...
        assert bus._store._subscribers[MockEvent][0].handler is handler2
        assert bus._store._subscribers[MockEvent][1].handler is handler1

    def test_subscribe_invalidates_cache(self, bus):
        def handler(event):
            pass

        bus._store._handler_cache = {(MockEvent,): []}
        bus.subscribe(MockEvent, handler)
    ...
    def test_unsubscribe(self, bus):
        ...
        assert len(bus._store._subscribers[MockEvent]) == 0
    ...
    def test_handler_caching(self, bus):
        ...
        handlers1 = bus._store.resolve_handlers(MockEvent)

        # Second call should use cache
        handlers2 = bus._store._get_cached_handlers(MockEvent)
    ...
    def test_cleanup_dead_handlers(self, bus):
        ...
        bus._store._cleanup_dead_handlers(MockEvent)
    ...
    def test_thread_safety_concurrent_subscribe(self, bus):
        ...
        threads = [
            threading.Thread(target=subscribe_handler, args=(i,)) for i in range(10)
        ]
...
class TestEventHandlerStore:
    ...
    def test_resolve_handlers_dedupes_same_bound_method_across_mro(self):
        store = EventHandlerStore(threading.RLock())
        owner = _HandlerOwner()

        store.subscribe(ParentEvent, owner.on_event, priority=80)
        store.subscribe(ChildEvent, owner.on_event, priority=10)

        resolved = store.resolve_handlers(ChildEvent)

        assert len(resolved) == 1
        assert resolved[0].handler_key == (
            "method",
            id(owner),
            id(owner.on_event.__func__),
        )
        assert resolved[0].priority == 10

    def test_resolve_handlers_keeps_same_method_on_different_instances(self):
        store = EventHandlerStore(threading.RLock())
        first_owner = _HandlerOwner()
        second_owner = _HandlerOwner()

        store.subscribe(ParentEvent, first_owner.on_event, priority=80)
        store.subscribe(ChildEvent, second_owner.on_event, priority=10)

        resolved = store.resolve_handlers(ChildEvent)

        assert len(resolved) == 2
        assert {wrapper.handler_key for wrapper in resolved} == {
            ("method", id(first_owner), id(first_owner.on_event.__func__)),
            ("method", id(second_owner), id(second_owner.on_event.__func__)),
        }
