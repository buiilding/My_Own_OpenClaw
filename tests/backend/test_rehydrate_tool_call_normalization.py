...
def test_normalize_tool_calls_skips_invalid_entries_and_preserves_alias_signature():
    normalized = normalize_tool_calls(
        [
            ...
            {"id": "call-2", "type": "function", "function": {"name": "browser", "arguments": "{\"action\":\"snapshot\"}", "thoughtSignature": "sig-2"}},
            {"id": "", "name": "bad"},
            "not-a-dict",
        ]
    )
    ...
    assert normalized == [
        {"id": "call-1", "name": "read_file", "arguments": {"path": "/tmp/a.txt"}},
        {"id": "call-2", "name": "browser", "arguments": {"action": "snapshot"}, "thought_signature": "sig-2"},
    ]


def test_extract_thought_signature_checks_multiple_sources():
    assert extract_thought_signature(
        {"other": "value"},
