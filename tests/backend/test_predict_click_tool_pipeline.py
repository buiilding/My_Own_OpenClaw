"""
Test file for predict_click tool that simulates the full LLM tool call pipeline.

This test demonstrates different scenarios for the predict_click tool:
1. Successful element detection and clicking
2. Missing element description (error)
3. Complex element descriptions with detailed visual cues

The predict_click tool uses vision-language models to analyze screenshots and find UI elements.
It automatically takes a screenshot and analyzes it to find the described element.

All scenarios simulate LLM tool call format:
{"functionCall": {"name": "predict_click", "args": {"element_description": "blue Save button"}}}
...
async def test_predict_click_button():
    """
    Test predict_click with a button description.
    """
    print("\n" + "="*60)
    print("TEST 1: PREDICT CLICK - SAVE BUTTON")
    print("="*60)

    description = "the blue Save button in the top-right corner of the window"
    await run_predict_click_test(element_description=description, expect_success=True)


async def test_predict_click_menu_item():
    """
    Test predict_click with a menu item description.
    """
    print("\n" + "="*60)
    print("TEST 2: PREDICT CLICK - MENU ITEM")
    print("="*60)

    description = "the File menu option in the top menu bar"
    await run_predict_click_test(element_description=description, expect_success=True)


async def test_predict_click_icon():
    """
    Test predict_click with an icon description.
    """
    print("\n" + "="*60)
    print("TEST 3: PREDICT CLICK - SEARCH ICON")
    print("="*60)

    description = "the search icon with a magnifying glass symbol"
    await run_predict_click_test(element_description=description, expect_success=True)


async def test_predict_click_missing_description():
    """
    Test predict_click with missing element description (error case).
    """
    print("\n" + "="*60)
    print("TEST 4: PREDICT CLICK - MISSING DESCRIPTION (ERROR)")
    print("="*60)

    await run_predict_click_test(expect_success=False, expect_error="element_description")


async def test_predict_click_complex_description():
    """
    Test predict_click with very detailed element description.
    """
    print("\n" + "="*60)
    print("TEST 5: PREDICT CLICK - COMPLEX DESCRIPTION")
    print("="*60)

    description = "the red close button with an X symbol in the top-right corner of the dialog box, next to the minimize and maximize buttons"
    await run_predict_click_test(element_description=description, expect_success=True)


async def run_predict_click_test(element_description: str = None, model_name: str = None, expect_success: bool = True, expect_error: str = None):
    """
    Run a single predict_click test with the given parameters.
    """
    ...
    import json
    llm_response = json.dumps({"functionCall": {"name": "predict_click", "args": args}})
    ...
    try:
        ...
        for i, result in enumerate(execution_result.tool_results, 1):
            ...
            if result.success:
                ...
                # For predict_click, verify the description is acknowledged (use enhanced content)
                validation_content = enhanced_llm_content.lower()
                if element_description and element_description.lower() in validation_content:
                    print("✅ Element description properly processed")
                elif element_description:
                    # Check for key terms from the description
                    key_terms = element_description.lower().split()
                    found_terms = [term for term in key_terms if term in validation_content]
                    if found_terms:
                        print(f"✅ Found description terms: {found_terms[:3]}...")
...
async def test_predict_click_tool_pipeline():
    """
    Run all predict_click test scenarios.
    """
    print("=" * 80)
    print("COMPREHENSIVE PREDICT CLICK TOOL PIPELINE TEST")
    print("=" * 80)

    print("\n📝 NOTE: Predict Click uses vision-language models to analyze screenshots.")
    print("It automatically takes screenshots and finds UI elements by description.")
    print("Tests focus on parameter validation and response structure.\n")

    # Test successful scenarios
    await test_predict_click_button()
    await test_predict_click_menu_item()
    await test_predict_click_icon()
    await test_predict_click_complex_description()

    # Test error case
    await test_predict_click_missing_description()

    print("\n" + "=" * 80)
    print("ALL PREDICT CLICK TOOL TESTS COMPLETED")
    print("=" * 80)


if __name__ == "__main__":
    # Run the test
    asyncio.run(test_predict_click_tool_pipeline())
