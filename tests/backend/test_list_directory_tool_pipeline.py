"""
Test file for list_directory tool that simulates the full LLM tool call pipeline.
...
async def test_list_directory_tool_pipeline():
    """
    Test the complete pipeline for a list_directory tool call.

    Simulates what happens when the LLM outputs:
    {"functionCall": {"name": "list_directory", "args": {"path": "."}}}
    """
    print("=" * 80)
    print("TESTING LIST DIRECTORY TOOL PIPELINE")
    print("=" * 80)

    # Step 1: Simulate LLM Response
    print("\n🔧 STEP 1: Simulating LLM Response")
    llm_response = '{"functionCall": {"name": "list_directory", "args": {"path": "."}}}'

    print(f"LLM Response: {llm_response}")

    # Step 2: Parse the Response
    print("\n📋 STEP 2: Parsing LLM Response")
    parser = create_test_parser(tool_names=["list_directory"])
...
if __name__ == "__main__":
    # Run the test
    asyncio.run(test_list_directory_tool_pipeline())
