"""
Test file for run_shell_command tool that simulates the full LLM tool call pipeline.

This test simulates what happens when the LLM generates a correct tool call format:
{"functionCall": {"name": "run_shell_command", "args": {"command": "echo 'Hello, World!'"}}}
...
async def test_run_shell_command_tool_pipeline():
    """
    Test the complete pipeline for a run_shell_command tool call.

    Simulates what happens when the LLM outputs:
    {"functionCall": {"name": "run_shell_command", "args": {"command": "echo 'Hello, World!'"}}}
    """
    print("=" * 80)
    print("TESTING RUN SHELL COMMAND TOOL PIPELINE")
    print("=" * 80)

    # Step 1: Simulate LLM Response
    print("\n🔧 STEP 1: Simulating LLM Response")
    llm_response = '{"functionCall": {"name": "run_shell_command", "args": {"command": "echo \'Hello, World!\'"}}}'

    print(f"LLM Response: {llm_response}")

    # Step 2: Parse the Response
    print("\n📋 STEP 2: Parsing LLM Response")
    parser = create_test_parser(tool_names=["run_shell_command"])
...
if __name__ == "__main__":
    # Run the test
    asyncio.run(test_run_shell_command_tool_pipeline())
