...
- Full-gate revalidation after backend LLM-provider standard completion-param helper dedupe + regression coverage (Phase 140 code slice):
  - frontend lint audits:
    - `cd frontend && npm run lint:audit` (pass; react-compiler + deprecation clean)
  - dead-code audit:
    - `cd frontend && npm run audit:knip` (pass)
...
- Full-gate revalidation after shared online-provider base extraction for backend LLM providers (Phase 141 code slice):
  - frontend lint audits:
    - `cd frontend && npm run lint:audit` (pass; react-compiler + deprecation clean)
  - dead-code audit:
    - `cd frontend && npm run audit:knip` (pass)
...
- Full-gate revalidation after Kimi provider migration to shared online-provider base (Phase 142 code slice):
  - frontend lint audits:
    - `cd frontend && npm run lint:audit` (pass; react-compiler + deprecation clean)
  - dead-code audit:
    - `cd frontend && npm run audit:knip` (pass)
...
- Full-gate revalidation after Local-provider inheritance migration to shared online-provider base (Phase 144 code slice):
  - frontend lint audits:
    - `cd frontend && npm run lint:audit` (pass; react-compiler + deprecation clean)
  - dead-code audit:
    - `cd frontend && npm run audit:knip` (pass)
...
- Full-gate revalidation after local-provider base-url/model-prefix dedupe (Phase 145 code slice):
  - frontend lint audits:
    - `cd frontend && npm run lint:audit` (pass; react-compiler + deprecation clean)
  - dead-code audit:
    - `cd frontend && npm run audit:knip` (pass)
...
- Full-gate revalidation after backend Kimi stream-helper dedupe + browser-schema literal alias refactors (Phase 146 code slice):
  - frontend lint audits:
    - `cd frontend && npm run lint:audit` (pass; react-compiler + deprecation clean)
  - dead-code audit:
    - `cd frontend && npm run audit:knip` (pass)
...
- Full-gate revalidation after sidecar Browser-Use LLM helper dedupe + message text-extraction dedupe (Phase 147 code slice):
  - frontend lint audits:
    - `cd frontend && npm run lint:audit` (pass; react-compiler + deprecation clean)
  - dead-code audit:
    - `cd frontend && npm run audit:knip` (pass)
...
- Full-gate revalidation after sidecar Browser-Use dedupe wave across LLM/chat helpers, HAR/header normalization, filesystem async sync path, DOM fallback page-info handling, session readiness counting, and timing logger resolution:
  - frontend lint audits:
    - `cd frontend && npm run lint:audit` (pass; react-compiler + deprecation clean)
  - dead-code audit:
    - `cd frontend && npm run audit:knip` (pass)
...
- Full-gate revalidation after sidecar observability helper refactor and regression coverage expansion:
  - frontend lint audits:
    - `cd frontend && npm run lint:audit` (pass; react-compiler + deprecation clean)
  - dead-code audit:
    - `cd frontend && npm run audit:knip` (pass)
...
- Full-gate revalidation after backend tool-preparer coordinate-resolution helper extraction:
  - frontend lint audits:
    - `cd frontend && npm run lint:audit` (pass; react-compiler + deprecation clean)
  - dead-code audit:
    - `cd frontend && npm run audit:knip` (pass)
