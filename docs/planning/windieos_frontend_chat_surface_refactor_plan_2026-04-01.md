...
3. Keep platform-specific screenshot visibility behavior explicit and minimal:
   ...
   - Windows and macOS never do hide/show, disappear/reappear, or capture-time restoration cycles, and the overlay screenshot-exclusion contract comes from phase-driven `setContentProtection(...)` during active loop phases.
...
- use window policy (`setContentProtection(...)`) during active loop phases instead of hide/show
...
6. On Windows and macOS, screenshot capture performs no overlay hide/show or disappear/reappear cycle, and overlay screenshot exclusion comes from phase-driven window content protection.
