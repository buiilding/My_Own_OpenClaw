...
| Websocket bearer auth and identity override | Backend websocket connection lifecycle | `backend/src/api/routes/websocket/connection.py`, `backend/src/api/schema.py` | `tests/backend/test_websocket_connection.py` | [WebSocket Connection Lifecycle](../gateway/websocket_connection_lifecycle.md) |
...
- `backend/src/api/schema.py` if handshake fields change.
