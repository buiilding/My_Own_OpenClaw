...
read_when:
  - When a change crosses backend, desktop host, renderer, local-runtime, local-runtime Python implementation, or SDK trust boundaries or touches auth, validation, credentials, permissions, or local tool execution.
...
| Local-runtime tool execution | local actions validate args and normalize results | `frontend/src/main/python/tools/**` | local action failure, unsafe path, shell/browser errors | sidecar pytest tests |
