...
read_when:
  ...
  - When debugging security-sensitive failures such as 401/1008, invalid IPC channels, hidden tools, permission denial, or sidecar execution errors.
...
| Sidecar tool execution | local actions validate args and normalize results | `frontend/src/main/python/tools/**` | local action failure, unsafe path, shell/browser errors | sidecar pytest tests |
...
- changes tool visibility, tool policies, or local sidecar execution.
