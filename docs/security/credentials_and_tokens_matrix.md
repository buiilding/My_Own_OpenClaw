...
| --- | --- | --- | --- | --- |
| Install token | Backend install auth service plus Electron main token persistence | Registered by `POST /api/install/register`, persisted by `ipc_install_auth_state.cjs` | Hosted `/api/*` middleware and `/ws` handshake | Provider API calls, runs worker auth, local tool authorization |
...
| Runs API key | Environment variables `WINDIE_RUNS_API_KEY`, `WINDIE_DEMO_API_KEY`, worker `WINDIE_VM_RUNS_API_KEY` | Env only | `/api/runs/*` route dependency and VM worker | General hosted REST auth |
...
| OpenAI Codex OAuth state | `frontend/src/main/openai_codex_oauth.cjs` and settings/UI where still supported | Provider-specific OAuth flow | Provider auth for supported models | Generic login, install auth, or app account auth |
...
| OAuth state | `frontend/src/main/openai_codex_oauth.cjs` | `tests/frontend/OpenAICodexOAuth.test.cjs` |
