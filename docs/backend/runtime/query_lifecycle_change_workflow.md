...
| --- | --- | --- | --- | --- |
| query is rejected before the model runs | websocket route, schema, or query handler | `backend/src/api/routes/websocket`, `backend/src/api/schema.py`, `backend/src/api/handlers/query.py` | [API WebSocket Docs Hub](../api/websocket/README.md), [Query Handler and Query Execution Service Runtime Reference](../api/handlers/query_handler_and_query_execution_service_runtime_reference.md) | `tests/backend/test_websocket_message_handler.py`, `tests/backend/test_query_execution_inputs.py` |
