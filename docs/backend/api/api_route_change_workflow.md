...
| Artifact upload/fetch route changes | Artifact route package and artifact store | `backend/src/api/routes/artifacts/*`, `backend/src/services/artifacts` | `tests/backend/test_artifact_routes.py`, `tests/backend/test_artifacts_store.py` | [Artifacts Route Package Split](artifacts_route_package_split_and_compatibility_export_contract_reference.md), [Artifact Change Workflow](../../desktop/artifact_change_workflow.md) |
...
| Docs-only API workflow updates | `./bin/docs-list`, `git diff --check`, focused Markdown link checks |
