...
Do not put service policy into API route functions or renderer code. Routes should validate and call services; agent code should orchestrate services through clear interfaces; client/local-runtime Python code should consume stable route/tool contracts.
