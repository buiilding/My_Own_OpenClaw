...
- hosted desktop runtimes may also supply pre-resolved `repo_instruction_messages` on query/rehydrate payloads so prompt construction does not depend on backend filesystem access to the user's local workspace
