...
| Tool schema missing, too broad, malformed, or different from capability policy | Backend tool registry, policy, prompt constructor, formatter metadata | `backend/src/tools`, `backend/src/tools/tool_selection.py`, `backend/src/llm/prompts/prompt_constructor.py`, `backend/src/api/processing/formatters/tool_schemas.py` | schema/policy tests, `tests/backend/test_system_use_schema_contract.py`, `tests/backend/test_outgoing_schema_contract.py` | [Tool Policy Profiles and Capabilities](../../../tools/tool_policy_profiles_and_capabilities.md), [Tool Contracts](../../../tools/tool_contracts.md) |
...
| Memory context injected into prompt is wrong or includes active conversation | Sidecar memory retrieval plus backend prompt constructor | sidecar memory search, Electron query payload, prompt constructor | sidecar memory tests, backend prompt constructor tests | [Memory Change Workflow](../../../memory/memory_change_workflow.md), [Context and Memory](../../../concepts/context_and_memory.md) |
...
| Sub-agent or SDK-authored session prompt differs from normal desktop prompt | SDK/session prompt builder | `backend/src/sdk/agents/session_builder.py`, `backend/src/core/services/agent_factory.py`, prompt modules | SDK/context/session initializer tests | [Tool Authoring](../../../sdk/tool_authoring.md), [Backend SDK Sub-Agent Helper Runtime](../../sdk/subagent_session_helper_runtime_reference.md) |
...
2. **Trace the producer.** Identify whether context came from backend session history, Electron query payload, sidecar memory, artifact store, backend tool registry, provider health, or SDK session builder.
...
| Docs-only prompt workflow updates | `./bin/docs-list`, `git diff --check`, focused Markdown link checks |
