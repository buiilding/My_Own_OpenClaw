...
| replayed chat image is missing | SDK replay adapter, renderer transcript projection, and attachment image resolver | `packages/windie-sdk-js/src/projections/legacyVisualAttachmentReplayAdapter.ts`, `frontend/src/renderer/infrastructure/transcript`, `frontend/src/renderer/app/runtime/desktopResolvedMessageScreenshotsRuntime.js`, SDK/local-runtime transcript store backed by local-runtime Python modules | [Memory Change Workflow](../memory/memory_change_workflow.md), [Transcript and Replay](../memory/transcript_and_replay.md) | frontend transcript/message attachment tests, SDK/local-runtime transcript tests |
