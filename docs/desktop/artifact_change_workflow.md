...
| tool-result screenshot is stripped or not stored | renderer tool execution payload and backend tool-result router | `frontend/src/renderer/infrastructure/services/ToolExecutionPayloads.ts`, `frontend/src/renderer/infrastructure/services/ToolExecutionBackendPayload.ts`, `backend/src/agent/tools/waiting/router.py` | [Tool Turn Change Workflow](../backend/agent/tool_turn_change_workflow.md), [Frontend Capture, Artifact Upload, and Payload Normalization Reference](../frontend/renderer/infrastructure/capture_artifact_upload_and_payload_normalization_reference.md) | `tests/frontend/ToolExecutionPayloads.test.ts`, `tests/frontend/ToolExecutionBackendPayload.test.ts`, backend tool-result tests |
...
- `screenshot_ref` compatibility path still works.
