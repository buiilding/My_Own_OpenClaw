...
- The compact pill side caps are drawn by a real rounded body layer, with the
  ...
  bump layer must use a small lower rounded bell-curve tab profile that overlaps
  the body border just enough to join as one shape without reading as a dome or
  peaked tent.
