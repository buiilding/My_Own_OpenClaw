...
- The compact pill side caps are drawn by a real rounded body layer, with the
  ...
  visible gray halo on white backgrounds. The body radius is finite rather than
  infinite so the compact state still clamps to a pill, while multiline
  composer growth keeps rounded-rectangle corners with vertical sides instead
  of turning the whole surface into an oval.
