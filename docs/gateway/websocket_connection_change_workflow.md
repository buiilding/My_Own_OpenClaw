...
| Electron handshake or reconnect drift | SDK desktop agent | `packages/windie-sdk-js/src/runtime/WindieDesktopAgent.ts`, `frontend/src/main/ipc.cjs`, frontend endpoint/auth code | frontend websocket/main IPC tests |
...
| Docs-only websocket changes | `./bin/docs-list`, `git diff --check`, focused Markdown link checks |
