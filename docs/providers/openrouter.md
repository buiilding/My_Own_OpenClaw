...
```bash
./scripts/test-backend tests/backend/test_openrouter_provider.py tests/backend/test_models_config.py -q
./scripts/test-backend tests/backend/test_model_service.py tests/backend/test_provider_factory_helpers.py -q
```
...
```bash
cd frontend
npm run test:ci -- ModelCardData.test.js ChatInterfaceWiring.test.jsx
```
