...
| SDK OCR/vision behavior changes | `backend/src/api/routes/sdk/*`, OCR/vision routers and service helpers, `backend/src/sdk`, hosted clients | `tests/backend/test_sdk_routes.py`, `tests/frontend/WindieSdkClient.test.ts`, [SDK Route Change Workflow](../sdk/sdk_route_change_workflow.md) |
| SDK memory embedding behavior changes | `packages/windie-sdk-js/src/runtime/ContextEnrichmentPipeline.ts`, `packages/windie-sdk-js/src/runtime/Agent.ts`, local-runtime memory store/search pipeline | SDK memory tests, `tests/sidecar/test_local_store_*.py`, [Sidecar Core Docs Hub](../frontend/sidecar/core/README.md) |
