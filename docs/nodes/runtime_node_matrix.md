...
| Electron main node | `frontend/src/main/index.cjs`, `frontend/src/main/ipc.cjs`, `frontend/src/main/windie_agent_host.cjs`, `frontend/src/main/main_process_*`, `frontend/src/main/surface_runtime.cjs`, `frontend/src/main/backend_endpoints.cjs` | Electron IPC, SDK desktop-agent host, local sidecar JSON-RPC/daemon bridge, OS window APIs | Electron app bootstrap composes windows, IPC handlers, SDK desktop agent, local sidecar supervisor, optional VM worker | renderer cannot send query, overlays do not open/close, config does not sync, backend URL wrong, sidecar never starts | main-process Jest tests, endpoint/config tests, overlay/window tests |
