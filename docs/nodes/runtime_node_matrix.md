...
| Electron main node | `frontend/src/main/index.cjs`, `frontend/src/main/ipc.cjs`, `frontend/src/main/windie_sdk_runtime.cjs`, `frontend/src/main/main_process_*`, `frontend/src/main/surface_runtime.cjs`, `frontend/src/main/backend_endpoints.cjs` | Electron IPC, SDK-runtime backend session adapter, local sidecar JSON-RPC/daemon bridge, OS window APIs | Electron app bootstrap composes windows, IPC handlers, SDK runtime, local sidecar supervisor, optional VM worker | renderer cannot send query, overlays do not open/close, config does not sync, backend URL wrong, sidecar never starts | main-process Jest tests, endpoint/config tests, overlay/window tests |
