...
| Electron main node | `frontend/src/main/index.cjs`, `frontend/src/main/ipc.cjs`, `packages/windie-sdk-js/src/runtime/WindieDesktopAgent.ts`, `frontend/src/main/main_process_*`, `frontend/src/main/surface_runtime.cjs`, `frontend/src/main/backend_endpoints.cjs` | Electron IPC, SDK desktop agent, local sidecar JSON-RPC/daemon bridge, OS window APIs | Electron app bootstrap composes windows, IPC handlers, SDK desktop agent, local sidecar supervisor, optional VM worker | renderer cannot send query, overlays do not open/close, config does not sync, backend URL wrong, sidecar never starts | main-process Jest tests, endpoint/config tests, overlay/window tests |
...
| Renderer node | `frontend/src/renderer/app/**`, `frontend/src/renderer/features/**`, `frontend/src/renderer/infrastructure/**` | renderer IPC facade, backend event consumers via main rebroadcast, browser DOM/UI state | React app mounts dashboard, overlays, providers, hooks, transcript stores, and display-only tool state | UI state does not match stream event, transcript replay drift, display tool state does not match SDK execution state, settings UI stale | renderer hook/store/component tests, event consumer tests |
