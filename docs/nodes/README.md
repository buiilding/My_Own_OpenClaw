...
| Electron main node | local desktop app main process | windows, overlay lifecycle, SDK-runtime adapter, local config, preload IPC handlers, local-runtime host/status context, VM worker mode | [Desktop and Sidecar Node](desktop_and_sidecar_node.md), [Frontend Main Docs Hub](../frontend/main/README.md) |
| Renderer node | local desktop renderer processes | dashboard, chat UI, overlay UI, voice UI, settings, transcript projection, tool runner UI state | [Desktop and Sidecar Node](desktop_and_sidecar_node.md), [Frontend Renderer Docs Hub](../frontend/renderer/README.md) |
...
| Python sidecar node | local Python sidecar daemon managed by SDK local runtime | filesystem/shell/computer/browser tools, local memory, system state, JSON-RPC methods, SDK helper clients | [Desktop and Sidecar Node](desktop_and_sidecar_node.md), [Local Runtime Python Implementation Docs Hub](../frontend/sidecar/README.md) |
