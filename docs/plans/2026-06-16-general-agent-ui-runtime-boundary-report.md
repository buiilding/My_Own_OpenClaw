...
- Latest completed slice: Electron main renderer chat query handling keeps the
  lower-level handler factory private to `ipc_chat_query_handlers.cjs`, so
  production and focused tests use `createChatQueryHandlerRuntime(...)` as the
  composition boundary while preserving renderer query/stop behavior.
