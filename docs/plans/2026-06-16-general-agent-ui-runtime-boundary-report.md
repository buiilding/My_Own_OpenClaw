...
- Latest completed slice: Electron main VM automated-query dispatch keeps the
  lower-level dispatcher private to `ipc_automated_query_dispatcher.cjs`, so
  production and focused tests use `createAutomatedQueryRuntime(...)` as the
  composition boundary while preserving VM query dispatch behavior.
