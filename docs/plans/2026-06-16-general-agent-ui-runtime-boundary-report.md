...
- Latest completed slice: Electron main Agent SDK connection-event handling
  keeps the lower-level connection-event and backend-fallback handlers private
  to `ipc_agent_connection_events.cjs`, so production and focused tests use
  `createAgentConnectionEventsRuntime(...)` as the composition boundary while
  preserving open/close/error adaptation, endpoint fallback selection, and
