...
- Latest completed slice: chat interface menu-dismiss, Stop shortcut, and
  thread-find shortcut subscriptions now route through
  `DesktopChatInterfaceBindingsRuntime`; `useChatInterfaceBindings(...)` keeps
  React lifecycle and callbacks local without owning raw `window` listener
  wiring or local shortcut predicate logic.
