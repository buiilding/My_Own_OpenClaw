...
- Latest completed slice: Electron main SDK-shaped invoke handling keeps the
  lower-level invoke handler and IPC registration helper private to
  `ipc_agent_sdk_command_handlers.cjs`, so production and focused tests use
  `createAgentSdkInvokeHandlerRuntime(...)` as the composition boundary while
  preserving `windie:invoke` command behavior.
