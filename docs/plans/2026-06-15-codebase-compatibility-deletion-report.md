...
- `9bcb5c08c refactor(frontend): keep stop shortcut helpers private`
  completed CD-080.
- `f4762cf31 refactor(frontend): trim layer log helper exports`
  completed CD-081.
- `fd1c9c454 refactor(frontend): keep permission verifiers private`
  completed CD-082.
- `04b9cf9b3 refactor(frontend): keep live turn typing logger private`
  completed CD-083.
- `e4dfd5246 refactor(frontend): trim local backend utility exports`
  completed CD-084.
- `1e45f3336 refactor(frontend): keep sidecar launch helpers private`
  completed CD-085.
...
- targeted `rg -n "verifyBrowserAutomationCapability|verifyInputControlCapability|verifyMicrophoneCapability|normalizePlatformScope|verifyScreenCaptureCapability|verifyShellExecutionCapability|verifyWorkspaceAccessCapability" frontend/src tests/frontend docs scripts bin --glob '!frontend/node_modules/**' --glob '!frontend/dist/**' --glob '!frontend/release/**' --glob '!frontend/python-runtime/**'`:
  remaining matches are private same-module helper calls, dependency-injection
  callback keys, or tests injecting verifier callbacks through public
  permission service entrypoints.
- `bin/windie test frontend -- PermissionService PermissionIpcRuntime AppPermissionGate useOnboardingPermissionActions permissionGrantEffects`:
  passed; 5 suites and 53 tests.
- `npm run audit:knip` in `frontend`: still exits 1 for broader existing
  dependency/export findings; unused exports dropped from 91 to 84 after
  removing the permission verifier helper exports.
- Migration note: no runtime, storage, settings, IPC, or persisted-data
...
- targeted `rg -n "DAEMON_LAUNCH_CONTEXT_ENV_KEYS|DEFAULT_DAEMON_DISCOVERY_PATH|buildSidecarDaemonEnv|buildSidecarLaunchContextFromEnv|writeSidecarDaemonLogLine" frontend/src tests/frontend docs scripts --glob '!frontend/node_modules/**' --glob '!frontend/dist/**' --glob '!frontend/release/**' --glob '!frontend/python-runtime/**'`:
  only private same-module helper definitions and uses remain.
- `bin/windie test frontend -- SdkSidecarLaunchOptions LocalBackendBridge.lifecycle LocalBackendBridge LocalBackendBridge.rpc`:
  passed; 8 suites and 67 tests.
- `npm run audit:knip` in `frontend`: still exits 1 for broader existing
  dependency/export findings; unused exports dropped from 80 to 75 after
  removing sidecar launch helper exports.
- Migration note: no runtime, storage, settings, IPC, websocket, JSON-RPC, or
...
- targeted `rg -n "isResponseOverlayPreflightGuardRef" frontend/src tests/frontend docs scripts --glob '!frontend/node_modules/**' --glob '!frontend/dist/**' --glob '!frontend/release/**' --glob '!frontend/python-runtime/**'`:
  only private same-module predicate definition and uses remain.
- `bin/windie test frontend -- ResponseOverlayPhaseHandler ResponseOverlayVisibilityPolicy SurfaceRuntime SdkLiveTurnSurfaceController`:
  passed; 4 suites and 60 tests.
- `npm run audit:knip` in `frontend`: still exits 1 for broader existing
  dependency/export findings; unused exports dropped from 66 to 65 after
  removing the response-overlay preflight guard predicate export.
- `bin/windie docs list`: passed.
- `git diff --check`: passed.
- Migration note: no runtime, storage, settings, IPC, websocket, or
  persisted-data migration is required. The same preflight guard ref still
  gates phase-driven hide/show behavior inside the response-overlay phase
  handler.

CD-091 validation:

- targeted `rg -n "CHAT_PILL_HIDE_REASON|CHAT_PILL_SHOW_REASON|formatChatPillVisibilityDecision|logChatPillVisibilityDecision" frontend/src tests/frontend docs scripts --glob '!frontend/node_modules/**' --glob '!frontend/dist/**' --glob '!frontend/release/**' --glob '!frontend/python-runtime/**'`:
  only private same-module helper definitions and uses remain.
- `bin/windie test frontend -- SurfaceRuntime MainProcessBootstrapRuntime MainWindowRuntime WindowVisibilityRuntime`:
  passed; 4 suites and 90 tests.
- `npm run audit:knip` in `frontend`: still exits 1 for broader existing
  dependency/export findings; unused exports dropped from 65 to 61 after
  removing chat-pill visibility helper exports.
- `bin/windie docs list`: passed.
- `git diff --check`: passed.
- Migration note: no runtime, storage, settings, IPC, websocket, or
  persisted-data migration is required. Chat-pill visibility persistence,
  startup suppression, user-hidden handling, diagnostics, and trace emission
  still route through `createSurfaceRuntime(...)`.
