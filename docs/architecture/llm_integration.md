...
- Stored assistant history now keeps replay-safe structured content alongside its legacy flattened text field, so provider replay can preserve `output_text`/`refusal` blocks without forcing compaction, transcript inspection, or token-cache code to stop using the existing string-backed history field.
