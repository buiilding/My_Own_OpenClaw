...
flowchart LR
  ...
  SdkRuntime --> IPC["Electron main local-runtime adapter"]
...
| SDK local execution | None directly; SDK executes on behalf of backend tool event | Normalized `tool_call` / `tool_bundle_call` events, request IDs, bundle IDs, tool-call IDs, local-runtime result envelopes | `packages/windie-sdk-js/src/tools/ToolExecutionCoordinator.ts`, `packages/windie-sdk-js/src/runtime/WindieDesktopAgent.ts`, `frontend/src/main/windie_agent_host.cjs` | Correlation IDs must survive event, execution, result relay, and transcript persistence. Late/stale turns should fail explicitly. |
