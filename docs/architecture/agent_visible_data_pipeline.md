...
| Tool visibility | Function/tool schemas and descriptions available to the model for this session | Backend registry output filtered by policy, client agent definition, provider health, and provider projection | `backend/src/tools`, `backend/src/tools/tool_policy.py`, `backend/src/tools/provider_health.py` | Desktop client/local-runtime Python code must not import backend schema code. If a local-runtime executable cannot execute a visible tool, add parity tests or hide it. |
