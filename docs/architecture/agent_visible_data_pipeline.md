...
read_when:
  ...
  - When changing query input, prompt context, tool schemas, tool calls, tool results, transcript replay, or transparency events across backend, Electron main, renderer, preload, and Python sidecar.
...
flowchart LR
  ...
  SidecarRpc --> LocalTool["Python sidecar local tool"]
...
| Tool visibility | Function/tool schemas and descriptions available to the model for this session | Backend registry output filtered by policy, client agent definition, provider health, and provider projection | `backend/src/tools`, `backend/src/tools/tool_policy.py`, `backend/src/tools/provider_health.py` | Desktop client/Python sidecar code must not import backend schema code. If a Python sidecar tool cannot execute a visible tool, add parity tests or hide it. |
...
| Tool preparation | Model intent plus any model-visible failure/recovery guidance | Resolved request IDs, coordinate/OCR/screenshot preparation data, executable args | `backend/src/agent/tools/preparation`, `backend/src/agent/tools/preparation/ocr`, `backend/src/agent/tools/preparation/coordinate_resolution` | Keep model-facing args separate from executable local-runtime args. If OCR resolves `click_ocr` to coordinates, document both shapes. |
...
| Sidecar JSON-RPC | None directly; Python sidecar is executor | JSON-RPC method params, snake_case Python fields, timeout/error envelopes | SDK local-runtime store/client code, `frontend/src/main/python/local_backend.py`, `frontend/src/main/python/core/ipc_protocol.py` | SDK local-runtime callers should centralize camelCase/snake_case and canonicalization. Avoid spreading fallback key logic into renderer callers. |
| Local tool result | Eventually: model-facing tool output after backend transforms it | Python sidecar `ToolResult` dict with `success`, `data`, `error`, screenshot/file/artifact fields | `frontend/src/main/python/tools/result.py`, Python sidecar implementation modules, Electron bridge helpers | Large or binary data should become artifact/file refs where possible, not repeated inline payloads through every transport. |
...
| --- | --- | --- |
| Runtime boundary adapter | It crosses backend, SDK/main, renderer, Python sidecar, trusted/untrusted, or JS/Python boundaries. | It only moves fields between two helpers in the same runtime without validation or ownership change. |
...
- Python sidecar code knows provider-specific model-facing schema rules
...
| The agent called a tool with odd args | Did the model see one shape but the Python sidecar execute another? | [Backend Tool Turn Change Workflow](../backend/agent/tool_turn_change_workflow.md), [Tool Execution Lifecycle](../tools/tool_execution_lifecycle.md) |
...
| Python sidecar executed but model never continued | Did `tool-result` return with the original request or bundle ID? | [Tool Execution Lifecycle](../tools/tool_execution_lifecycle.md), [Tool Result Ingress](../backend/tools/tool_result_ingress_and_storage_reference.md) |
...
| Tool execution path | backend tool-turn tests, SDK coordinator/runtime tests, main IPC tool-router tests, Python sidecar tool tests |
