...
- **SDK debug routes are first-class app routes**: the same SDK router now exposes developer-facing `/api/sdk/debug/*` introspection APIs for model catalogs, system-prompt inspection, tool-schema snapshots, per-tool capabilities, and prompt previews built from the backend’s real prompt/tool pipeline.
