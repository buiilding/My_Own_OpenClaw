        ...
        "tool_calls": [
          {
            ...
            "function": {
              "name": "system_use",
              "arguments": "{\"tool\":\"run_shell_command\",\"explanation\":\"check current workspace\",\"arguments\":{\"command\":\"pwd\",\"run_in_background\":false}}"
            }
          }
        ]
      }
...
- backend validation rejects malformed wrapper/tool payloads before frontend execution and returns repair guidance to the model on the next iteration
- `browser` validation keeps a flat transport schema and adds targeted repair guidance for malformed calls missing top-level `action`
...
Unified wrapper normalization contract before execution:

- `computer_use` is normalized to concrete computer tools (`mouse_control`, `keyboard_control`, `screenshot`, `scroll_control`, `switch_tab`, `wait`)
...
- `scroll_control` vertical actions (`scroll_up`/`scroll_down`) default to an executor-owned 5-click amount across Windows, macOS, and Linux; optional `clicks` remains available as an explicit literal override.
...
- **system_use**: Unified wrapper for `run_shell_command`, `replace`, `read_file`, `get_system_stats`, and `get_open_windows` (canonical envelope: `tool`, top-level `explanation`, `arguments`)
- **open_app**: Launch GUI app detached from sidecar lifecycle with optional window/screenshot verification
- **run_shell_command**: Execute shell command (supports `yield_after_seconds` + `env` overrides; defaults to the selected workspace folder when one has been opened in WindieOS and otherwise to the user home directory when `directory` is omitted; foreground `llm_content` is truncated by default to ~10,000 tokens with marker support via `max_output_tokens`; use `process` for background sessions)
