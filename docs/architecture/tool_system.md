...
- `browser` validation now checks both the broad transport schema and the strict model-facing `oneOf` action branch, so empty browser payloads or mixed-branch calls (for example `action="search"` with `url`) fail early with branch-specific retry guidance
