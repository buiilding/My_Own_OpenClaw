...
| renderer loads but sidecar tools fail | Electron main local-runtime bridge | local-runtime readiness logs, `frontend/src/main/sidecar/local_runtime_bridge.cjs`, `WINDIE_SIDECAR_LOG_LEVEL=DEBUG` |
...
| hosted route returns `502` | Cloudflare/origin service | check backend and cloudflared user services |
| hosted route returns `401` | install auth or runs key | verify token/key and target host |
