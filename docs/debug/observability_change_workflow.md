...
| app startup, sidebar chat-list, or non-turn diagnostics that must survive restart | Electron main app diagnostics store plus producer runtime | `frontend/src/main/diagnostics/app_diagnostics_store.cjs`, SDK/runtime producer, sidecar producer | [Runtime Traces](runtime_traces.md), [Storage Persistence Change Workflow](../architecture/storage_persistence_change_workflow.md) | app diagnostics store tests plus focused producer tests |
| tool execution or tool screenshot debug output | SDK desktop-agent tool routing and sidecar capture | `packages/windie-sdk-js/src/tools`, `packages/windie-sdk-js/src/runtime/WindieDesktopAgent.ts`, `frontend/src/main/sidecar/local_backend_bridge_screenshot_attachment.cjs` | [Tool Execution Lifecycle](../tools/tool_execution_lifecycle.md), [Runtime Traces](runtime_traces.md) | SDK desktop-agent tests, sidecar screenshot tests |
