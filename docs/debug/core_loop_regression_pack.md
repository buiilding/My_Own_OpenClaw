...
| User-included images render in the optimistic pending user row and stay visible through SDK text-only user-row handoff before artifact metadata arrives. | `ChatMessageSender.test.tsx`, `ChatStore.test.ts`, `ConversationRuntimeProjectionStream.test.ts` |
