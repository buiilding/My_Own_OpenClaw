...
| Normal dashboard sends keep the renderer pending user row visible through awaiting-only SDK current-turn projections; typing/busy state may come from SDK, but pending is not cleared until visible SDK rows, first-class `ConversationView`, legacy visible current-turn content, or terminal lifecycle replaces it. | `ConversationRuntimeProjectionStream.test.ts`, `DesktopVisibleTurnLifecycleRuntime.test.js`, `DesktopCurrentTurnWorkspaceRuntime.test.ts` |
