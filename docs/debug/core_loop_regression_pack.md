...
| User-included images render in the optimistic pending user row, stay visible through SDK text-only user-row handoff, and keep their rendered inline source through same-turn artifact-backed source fetches and message remounts. | `ChatMessageSender.test.tsx`, `ChatStore.test.ts`, `ConversationRuntimeProjectionStream.test.ts`, `DesktopResolvedMessageScreenshotsRuntime.test.jsx` |
