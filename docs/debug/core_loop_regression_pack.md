...
| User-included images render in the optimistic pending user row, stay visible through repeated SDK text-only user-row handoffs and dashboard-open display loads, and keep their rendered inline source through same-turn artifact-backed source fetches and message remounts. | `ChatMessageSender.test.tsx`, `ChatStore.test.ts`, `DesktopConversationDisplayProjection.test.ts`, `ConversationRuntimeProjectionStream.test.ts`, `UseDashboardConversations.test.jsx`, `DesktopResolvedMessageScreenshotsRuntime.test.jsx` |
