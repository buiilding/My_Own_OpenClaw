...
| Tool-call rows materialized in SDK display rows are not duplicated by same-turn first-class `ConversationView` live entries with the same nested tool-call/request identity. | `MessagePresentationPipeline.test.js` |
