...
| User-included images render in the optimistic pending user row, stay visible through SDK text-only user-row handoff, and keep their rendered inline source until same-turn artifact-backed image sources resolve. | `ChatMessageSender.test.tsx`, `ChatStore.test.ts`, `ConversationRuntimeProjectionStream.test.ts`, `DesktopResolvedMessageScreenshotsRuntime.test.jsx` |
