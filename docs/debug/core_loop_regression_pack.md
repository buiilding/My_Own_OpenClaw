...
| Empty SDK display-row projections remain conversation-scoped, so editing the first user row clears the old visible suffix before the replacement turn appends new rows. | `IpcDirectWakeUpAgentAdapter.test.cjs`, `DesktopConversationRuntimeEventClient.test.ts`, `ConversationReplayActions.test.jsx` |
