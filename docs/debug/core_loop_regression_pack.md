...
| Empty SDK display-row projections remain conversation-scoped, and SDK user display rows preserve turn refs, so editing the first user row clears the old visible suffix before the replacement turn appends new rows without duplicating the pending optimistic row. | `IpcDirectWakeUpAgentAdapter.test.cjs`, `DesktopConversationRuntimeEventClient.test.ts`, `SdkDisplayChatMessageProjection.test.ts`, `DesktopConversationDisplayProjection.test.ts`, `ConversationReplayActions.test.jsx` |
