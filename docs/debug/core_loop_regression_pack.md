...
| Empty SDK display-row projections remain conversation-scoped, SDK user display rows preserve turn refs, and SDK display timeline loads include same-revision replacement send rows, so editing the first user row clears the old visible suffix before the replacement turn appends new rows without duplicating display-row or pending optimistic user rows. | `AgentSdkConversationRuntime.test.ts`, `IpcDirectWakeUpAgentAdapter.test.cjs`, `DesktopConversationRuntimeEventClient.test.ts`, `SdkDisplayChatMessageProjection.test.ts`, `DesktopConversationDisplayProjection.test.ts`, `ConversationReplayActions.test.jsx` |
