...
| Empty SDK display-row projections remain conversation-scoped, SDK user display rows preserve turn refs, SDK source/CJS display timeline loads include same-revision replacement send rows, and renderer edit/resend publishes only the retained prefix before accepting the edited pending turn, so editing the first user row clears the old visible suffix before the replacement turn appends new rows without duplicating display-row or pending optimistic user rows. | `AgentSdkConversationRuntime.test.ts`, `AgentSdkCjsConversationRuntime.test.cjs`, `IpcDirectWakeUpAgentAdapter.test.cjs`, `DesktopConversationRuntimeEventClient.test.ts`, `SdkDisplayChatMessageProjection.test.ts`, `DesktopConversationDisplayProjection.test.ts`, `ConversationReplayActions.test.jsx` |
