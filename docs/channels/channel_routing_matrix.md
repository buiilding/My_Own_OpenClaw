...
| --- | --- | --- | --- | --- | --- |
| Dashboard chat query | dashboard composer submit | renderer `windie:send` IPC -> Electron main SDK customer -> SDK agent -> `/ws` `query` | `backend/src/api/handlers/query.py`, `backend/src/api/services/query_execution.py` | `frontend/src/renderer/features/chat/**`, `frontend/src/main/ipc.cjs`, Windie SDK desktop agent | backend query tests, frontend chat/IPC tests |
...
| Settings/model messages | dashboard settings or startup sync | renderer `windie:update-settings` / `windie:list-models` IPC -> Electron main SDK customer -> SDK agent -> `/ws` non-query handlers | `backend/src/api/handlers/settings.py`, model/list handlers | settings provider, `ipc_settings_sync.cjs`, SDK desktop agent | settings ACK and model-list tests |
| Stop query | user stop, VM stop control | renderer `windie:stop` or VM worker -> SDK agent -> `/ws` `stop-query` | `backend/src/api/handlers/stop_query.py` | renderer controls, SDK backend transport adapter, `vm_worker_runtime.cjs` | stop/cancellation tests, VM worker tests |
| Tool result ingress | SDK desktop agent local execution completion | SDK agent -> Electron local adapter -> sidecar daemon -> SDK agent -> `/ws` `tool-result` or `tool-bundle-result` | backend tool-result receiver/router/history commit | SDK desktop agent, sidecar daemon, renderer SDK display rows | backend tool-result tests, SDK/IPC router tests, sidecar tests |
| Backend stream events | agent loop output | backend `/ws` outgoing event -> SDK agent normalization/projection -> Electron main -> renderer `windie:conversation-event`, `windie:rows`, `windie:current-turn` | formatter registry, outgoing schemas, SafeWebSocket sender | SDK desktop agent, main event fan-out, renderer stream consumers | formatter/schema tests, renderer event guards |
...
| Local sidecar tool | model-visible tool call | backend emits tool-call -> SDK main runtime executes via sidecar daemon and fans out display-only renderer event | backend schema/preparation/waiting/history | SDK main runtime, `local_backend_bridge.cjs`, sidecar daemon/tools | schema parity, sidecar tool, SDK/IPC router tests |
...
| Tool call appears but local action does not run | SDK runtime tool router or sidecar executor | [Sidecar and Tool Channels](sidecar_and_tool_channels.md), [Tool Execution Lifecycle](../tools/tool_execution_lifecycle.md) |
