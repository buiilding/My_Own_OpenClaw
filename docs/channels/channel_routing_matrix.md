...
| --- | --- | --- | --- | --- | --- |
| Dashboard chat query | dashboard composer submit | renderer `windie:invoke` command `conversation.send` -> Electron main SDK customer -> SDK agent -> `/ws` `query` | `backend/src/api/handlers/query.py`, `backend/src/api/services/query_execution.py` | `frontend/src/renderer/features/chat/**`, `frontend/src/main/ipc.cjs`, Windie SDK desktop agent | backend query tests, frontend chat/IPC tests |
...
| Settings/model messages | dashboard settings or startup sync | renderer `windie:invoke` commands `settings.update` / `models.list` -> Electron main SDK customer -> SDK agent -> `/ws` non-query handlers | `backend/src/api/handlers/settings.py`, model/list handlers | settings provider, `ipc_settings_sync.cjs`, SDK desktop agent | settings ACK and model-list tests |
| Stop query | user stop, VM stop control | renderer `windie:invoke` command `conversation.stop` or VM worker -> SDK agent -> `/ws` `stop-query` | `backend/src/api/handlers/stop_query.py` | renderer controls, SDK backend transport adapter, `vm_worker_runtime.cjs` | stop/cancellation tests, VM worker tests |
