...
| --- | --- | --- | --- | --- | --- |
| Dashboard chat query | dashboard composer submit | renderer `windie:invoke` command `conversation.send` -> Electron main Agent SDK host -> SDK agent -> `/ws` `query` | `backend/src/api/handlers/query.py`, `backend/src/api/services/query_execution.py` | `frontend/src/renderer/features/chat/**`, `frontend/src/main/ipc.cjs`, Agent SDK desktop host | backend query tests, frontend chat/IPC tests |
...
| Settings/model messages | dashboard settings or startup sync | renderer `windie:invoke` commands `settings.update` / `models.list` -> Electron main Agent SDK host -> SDK agent -> `/ws` non-query handlers | `backend/src/api/handlers/settings.py`, model/list handlers | settings provider, `ipc_settings_sync.cjs`, Agent SDK desktop host | settings ACK and model-list tests |
