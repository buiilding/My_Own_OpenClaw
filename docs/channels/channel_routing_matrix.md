...
| Tool result ingress | SDK main runtime local execution completion | Electron main SDK runtime -> sidecar daemon -> `/ws` `tool-result` or `tool-bundle-result` | backend tool-result receiver/router/history commit | SDK main runtime, sidecar daemon, renderer display-only tool events | backend tool-result tests, SDK/IPC router tests, sidecar tests |
