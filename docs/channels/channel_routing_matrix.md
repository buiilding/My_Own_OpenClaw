...
| Tool result ingress | SDK/main local execution completion | SDK/main local runtime -> Electron local adapter -> Python sidecar daemon -> SDK/main local runtime -> `/ws` `tool-result` or `tool-bundle-result` | backend tool-result receiver/router/history commit | SDK/main local runtime, Python sidecar daemon, renderer SDK display rows | backend tool-result tests, SDK/IPC router tests, sidecar tests |
