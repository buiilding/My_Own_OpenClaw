...
2. Confirm the Agent SDK host publishes the event through the expected SDK projection or typed side-channel and does not swallow it as a special case.
...
- Confirm the Agent SDK host does not overwrite conversation context with stale fallback.
