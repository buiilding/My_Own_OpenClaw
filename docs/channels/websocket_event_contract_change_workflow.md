...
| tool-call, tool-output, or tool-bundle event changes | backend tool formatter plus SDK desktop agent and renderer display consumers | backend tool formatters, `packages/windie-sdk-js/src/runtime/WindieDesktopAgent.ts`, `packages/windie-sdk-js/src/tools/ToolExecutionCoordinator.ts`, renderer tool message utilities | [Tool Schema and Policy Change Workflow](../tools/tool_schema_policy_change_workflow.md), [Tool Execution Lifecycle](../tools/tool_execution_lifecycle.md) | backend tool formatter/result tests plus SDK desktop-agent and renderer display tests |
...
| `audio-chunk` or TTS stream changes | backend TTS/audio path and renderer audio parser | `backend/src/api/processing/tts`, `frontend/src/renderer/features/chat/utils/backendAudioEvents.js` | [Voice Audio Change Workflow](voice_audio_change_workflow.md), [Backend TTS Manager](../backend/api/processing/tts/tts_manager_audio_stream_and_cleanup_reference.md) | backend TTS tests plus `tests/frontend/BackendAudioEvents.test.js` |
...
- Confirm Electron main does not overwrite conversation context with stale fallback.
