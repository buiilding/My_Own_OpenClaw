...
| tool-call, tool-output, or tool-bundle event changes | backend tool formatter plus SDK desktop agent and renderer display consumers | backend tool formatters, `packages/windie-sdk-js/src/tools/ToolExecutionCoordinator.ts`, `frontend/src/main/windie_agent_host.cjs`, renderer tool message utilities | [Tool Schema and Policy Change Workflow](../tools/tool_schema_policy_change_workflow.md), [Tool Execution Lifecycle](../tools/tool_execution_lifecycle.md) | backend tool formatter/result tests plus SDK desktop-agent and renderer display tests |
