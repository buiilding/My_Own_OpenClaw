...
| tool-call, tool-output, or tool-bundle event changes | backend tool formatter plus renderer tool runner | backend tool formatters, `useToolRunner.ts`, renderer tool message utilities | [Tool Schema and Policy Change Workflow](../tools/tool_schema_policy_change_workflow.md), [Tool Execution Lifecycle](../tools/tool_execution_lifecycle.md) | backend tool formatter/result tests plus frontend `ToolRunner`/`ToolExecution` tests |
...
Tool events are both UI state and execution requests.
...
4. Update renderer tool message rendering and tool runner dispatch.
