...
- **Lower-noise stderr forwarding**: Electron main now forwards local-runtime stderr lines by severity (`WARNING+` by default) with opt-in verbose pass-through via `WINDIE_VERBOSE_LOCAL_RUNTIME_STDERR=1`.
