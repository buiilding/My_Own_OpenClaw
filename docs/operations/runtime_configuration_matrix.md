...
| Sidecar | Local tool runtime flags, backend URL used by sidecar memory/API clients, worker counts, browser runtime knobs | Sidecar process startup; tool calls read some env values lazily | `frontend/src/main/python/local_backend.py`, `frontend/src/main/python/core/backend_config.py`, `frontend/src/main/python/core/executors.py`, `frontend/src/main/python/tools/**` |
...
| `ocr_backend`, `vision_backend`, `embedding_provider` | Backend `AppConfig` | Chooses local, remote HTTP, vendor, or disabled inference routes | `backend/src/core/config/models.py`, `backend/src/services/screen_grounding`, `backend/src/services/embedding*` |
...
| `WINDIE_VERBOSE_SIDECAR_STDERR` | Electron main env | Forwards all sidecar stderr when `1`; default is severity-filtered | `frontend/src/main/local_backend_bridge.cjs` |
