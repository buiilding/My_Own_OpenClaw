...
`scripts/deploy/update-remote-backend` to the host, updates the remote checkout,
...
`scripts/deploy/update-remote-backend` refuses to deploy when the remote checkout
...
```bash
WINDIE_BACKEND_SSH_HOST=windie-prod scripts/dev/backend-logs --service both --tail 200
