...
- SDK/main dispatch lives under `packages/windie-sdk-js/src/index.ts`, `packages/windie-sdk-js/src/runtime/WindieDesktopAgent.ts`, `packages/windie-sdk-js/src/tools`, and `frontend/src/main/windie_agent_host.cjs`. It should preserve correlation, bundle semantics, formatted model content, screenshot/system-state inclusion rules, and result envelopes.
...
| Tool is visible but never reaches the sidecar | SDK desktop agent host or Electron bridge | `frontend/src/main/windie_agent_host.cjs`, `frontend/src/main/local_backend_bridge_execute_tool_runtime.cjs` | `tests/frontend/WindieAgentHost.test.cjs`, bridge lifecycle/RPC tests |
...
| Tool result reaches UI but not backend continuation | SDK result envelope or backend result ingress | `packages/windie-sdk-js/src/index.ts`, `frontend/src/main/windie_agent_host.cjs`, `backend/src/api/handlers/tool_result.py` | SDK/main runtime and backend tool-result ingress docs/tests |
...
3. Trace the request through SDK main runtime and Electron.
   - SDK desktop agent host: `frontend/src/main/windie_agent_host.cjs`.
...
8. Update docs next to behavior.
   ...
   - Update [Filesystem Read and Replace Runtime Reference](../frontend/sidecar/tools/filesystem_read_replace_runtime_reference.md) for sidecar read/edit internals.
   - Update [Shell and Process Session Runtime Reference](../frontend/sidecar/tools/shell_and_process_session_runtime_reference.md) for sidecar shell/session internals.
   - Update [Sidecar Tool Change Workflow](../frontend/sidecar_tool_change_workflow.md) when the cross-runtime path changes.
...
| Sidecar `read_file` behavior | `./scripts/python-in-env sidecar pytest tests/sidecar/test_read_file_tool.py` |
...
| SDK/main dispatch/result envelope behavior | `cd frontend && npm run test -- WindieSdkDesktopAgent WindieAgentHost` |
...
3. Check `windie_agent_host.cjs` provides the local runtime adapter and dispatches the local runtime call.
