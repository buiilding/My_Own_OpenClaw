...
| SDK/main drops fields, result ids, artifacts, screenshots, or bundle metadata | SDK desktop agent and main local-runtime bridge | `packages/windie-sdk-js/src/index.ts`, `packages/windie-sdk-js/src/runtime/WindieDesktopAgent.ts`, `frontend/src/main/windie_agent_host.cjs` | [Tool Execution Lifecycle](tool_execution_lifecycle.md), [Sidecar and Tool Channels](../channels/sidecar_and_tool_channels.md) | SDK desktop-agent tests, host tests, backend tool-result tests |
...
| SDK/main dispatch/result envelope | focused `cd frontend && npm run test -- WindieSdkDesktopAgent WindieAgentHost ToolResult` tests |
