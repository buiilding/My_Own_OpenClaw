---
summary: "Frontend renderer infrastructure docs hub for audio playback queue runtime, `BackendEndpointStore` capture/artifact URL behavior, removed `SurfaceOrchestrator` service routing, display projection boundaries, incoming text normalization contracts, and chat markdown rendering owner routing."
read_when:
  ...
  - When debugging screenshot capture/artifact URL drift, `BackendEndpointStore` routing, stale `SurfaceOrchestrator` references, display projection drift, malformed renderer-side service payloads, or chat markdown rendering owner routing.
...
- [Removed SurfaceOrchestrator Reference](removed_surface_orchestrator_reference.md)
