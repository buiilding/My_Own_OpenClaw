...
| Tool-call or tool-output cards render incorrectly after backend events | SDK current-turn projection and message payload projection | `features/chat/hooks/useConversationRuntimeProjectionStream.ts`, `app/runtime/desktopCurrentTurnMessageRuntime.js`, `app/runtime/desktopSdkDisplayChatMessageProjectionRuntime.ts`, `features/chat/payloads/*`, `infrastructure/transcript/*` | `tests/frontend/ChatStreamThinkingStatus.state.test.tsx`, `tests/frontend/ChatBoxResponse.state.test.jsx`, `tests/frontend/DesktopCurrentTurnMessageRuntime.test.js`, `tests/frontend/SdkDisplayChatMessageProjection.test.ts` | [Tool Call/Output Rendering](chat/payloads/tool_call_output_and_transparency_section_rendering_reference.md) |
...
| SDK tool coordination and renderer tool display | `cd frontend && npm run test -- AgentSdkConversationRuntime LocalRuntimeExecuteToolRuntime DesktopCurrentTurnMessageRuntime SdkDisplayChatMessageProjection ToolCallMessageState` |
