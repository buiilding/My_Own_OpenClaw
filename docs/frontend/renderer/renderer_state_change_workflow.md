...
Use this workflow for UI and renderer-state work. The renderer owns presentation, local state projection, user interaction hooks, frontend config persistence, transcript projection, and dispatch to existing IPC/backend clients. It does not own backend event production, Electron window policy, preload channel exposure, sidecar tool execution, or hosted auth decisions.

The goal is to fix the state owner, not the nearest component. If a malformed backend event causes bad UI, fix the backend event contract. If a sidecar result is malformed, fix the sidecar/main bridge. If the renderer projection is stale or over-broad, fix the renderer store/hook/selector layer.

## Fast Owner Map

| Symptom or request | Renderer owner | First source roots | First tests | First docs |
| --- | --- | --- | --- | --- |
| Message list, input, pending send, screenshot attachment, or send button behavior changes | Chat send path and presentation | `frontend/src/renderer/features/chat/hooks/useChatMessageSender.ts`, `frontend/src/renderer/features/chat/components/MessageInput.jsx`, `frontend/src/renderer/features/chat/utils/messageSender/*` | `tests/frontend/ChatMessageSender.test.tsx`, `tests/frontend/ChatMessageSenderPayloads.test.ts`, `tests/frontend/ChatMessageSenderUtils.test.ts` | [Message Send Surface Policy](chat/message_send_surface_policy_and_screenshot_capture_reference.md) |
| Streamed text, thinking text, token counts, completion, or stale-turn filtering is wrong | Chat stream ingestion and chat store | `features/chat/hooks/useChatStream.ts`, `features/chat/hooks/chatStream/*`, `features/chat/utils/chatStream/*`, `features/chat/stores/chatStore.ts` | `tests/frontend/ChatStream*.test.ts`, `tests/frontend/ChatStore.test.ts` | [Chat Stream and Tool Execution](chat_stream_and_tool_execution_reference.md), [Stream Event State Machine](../runtime/stream_event_state_machine.md) |
| Tool-call or tool-output cards render incorrectly after backend events | Stream tool handlers and message payload projection | `features/chat/hooks/chatStream/useChatStreamToolHandlers.ts`, `features/chat/utils/chatStream/chatStreamToolMessages.ts`, `features/chat/payloads/*`, `infrastructure/transcript/*` | `tests/frontend/ChatStreamToolHandlers.test.ts`, `tests/frontend/ChatStreamToolMessages.test.ts`, `tests/frontend/ToolOutputMessageState.test.ts` | [Tool Call/Output Rendering](chat/payloads/tool_call_output_and_transparency_section_rendering_reference.md) |
| Frontend-executed tool result never returns, returns twice, or targets stale turn | Tool runner and infrastructure service | `features/chat/hooks/useToolRunner.ts`, `features/chat/hooks/useToolRunnerBackendListener.ts`, `features/chat/utils/toolRunner/*`, `infrastructure/services/toolExecution/*` | `tests/frontend/ToolRunnerExecutionState.test.ts`, `tests/frontend/ToolExecutionService*.test.ts`, `tests/frontend/ToolExecutionResultDispatch.test.ts` | [Tool Execution Service and Hook](infrastructure/tool_execution_service_and_hook_runtime_reference.md), [Sidecar Tool Change Workflow](../sidecar_tool_change_workflow.md) |
| Minimal pill, response overlay, stop button, or busy state gets stuck | Current-turn projection and overlay state | `features/chat/hooks/useCurrentTurnPresentationState.js`, `features/chat/hooks/useChatLoopUiState.js`, `features/chat/utils/state/*`, `features/chat/utils/overlay/*` | `tests/frontend/ChatLoopUiState*.test.*`, `tests/frontend/StreamPhaseState.test.js`, `tests/frontend/ChatBoxResponse*.test.jsx` | [Chat Loop UI State](chat/loop_ui_state_disconnect_recovery_and_surface_projection_reference.md), [Response Overlay Phase](overlays/response_overlay_phase_and_tool_ghost_runtime_reference.md) |
| New chat, resume conversation, active conversation, or wrong transcript target changes | Chat workspace state plus transcript session sync | `features/chat/stores/chatStore.ts`, `features/chat/utils/session/*`, `features/dashboard/components/ChatGptDashboardShell.jsx`, `infrastructure/transcript/transcriptSessionRuntime.ts` | `tests/frontend/NewChatSession.test.ts`, `tests/frontend/ResetActiveChatSession.test.ts`, `tests/frontend/TranscriptSessionState.test.ts`, `tests/frontend/ChatWorkspaceState.test.ts` | [Chat Store and New Session Rotation](chat/chat_store_state_and_new_session_rotation_reference.md), [Transcript Session and Rehydrate](transcript_session_and_rehydrate_reference.md) |
| Dashboard conversation list, sidebar search, recent chat loading, or resume flow changes | Dashboard shell and conversation hooks | `features/dashboard/components/DashboardShell.jsx`, `features/dashboard/components/DashboardSidebar.jsx`, `features/dashboard/hooks/useDashboardConversations.js`, `features/dashboard/utils/conversationGroups.js` | `tests/frontend/DashboardConversationLoad.test.js`, `tests/frontend/DashboardSidebar.test.jsx`, `tests/frontend/ChatGptDashboardShell.test.jsx` | [Renderer Dashboard Hub](dashboard/README.md) |
| Memory, models, settings, usage, or API-key panel state changes | Dashboard section component plus section helper | `features/dashboard/components/sections/*`, `features/dashboard/utils/*`, `features/settings/hooks/useSettingsManagement.ts` | `tests/frontend/MemorySectionState.test.js`, `tests/frontend/SettingsSection.test.jsx`, `tests/frontend/ChatModelOptions.test.js`, `tests/frontend/SettingsManagementHook.test.ts` | [Dashboard Sections Hub](dashboard/sections/README.md), [Renderer Settings Hub](settings/README.md) |
| Settings value persists incorrectly or backend config sync is wrong | App config provider and settings sync | `app/providers/AppConfigProvider.jsx`, `app/providers/appConfigPersistence.js`, `app/providers/appConfigBackendSync.js`, `features/settings/hooks/useSettingsManagement.ts` | `tests/frontend/AppConfigProvider*.test.tsx`, `tests/frontend/IpcSettingsSync.test.cjs`, `tests/frontend/SettingsManagementHook.test.ts` | [Settings Sync Change Workflow](../runtime/settings_sync_change_workflow.md), [Frontend Config Filter](settings/config/frontend_config_filter_storage_and_provider_merge_runtime_reference.md) |
...
| Replay, transcript load, pending queue, or stored message shape changes | Transcript infrastructure and replay projection | `infrastructure/transcript/*`, `features/chat/utils/session/transcriptMessagePayload.js`, `features/chat/utils/conversationReplayToolMessages.js` | `tests/frontend/Transcript*.test.*`, `tests/frontend/StoredTranscript*.test.js`, `tests/frontend/ConversationReplayState.test.ts` | [Renderer Transcript Hub](transcript/README.md), [Transcript Session and Rehydrate](transcript_session_and_rehydrate_reference.md) |
...
- Do not repair sidecar tool payloads in chat components. Fix `ToolExecutionService`, the main bridge mapper, or sidecar executor depending on where the payload first becomes wrong.
- Do not put raw filesystem, shell, browser, or computer-use execution in renderer code. Renderer requests execution through the tool runner or IPC.
...
- Keep stale-turn guards before tool execution and before transcript side effects.
...
| Chat send path | `cd frontend && npm run test -- ChatMessageSender ChatMessageSenderPayloads ChatMessageSenderUtils` |
| Stream ingestion/formatting | `cd frontend && npm run test -- ChatStream` |
| Tool runner/result dispatch | `cd frontend && npm run test -- ToolRunner ToolExecution` |
| Overlay/current-turn projection | `cd frontend && npm run test -- ChatLoopUiState ChatBoxResponse StreamPhaseState` |
| Dashboard shell/sidebar | `cd frontend && npm run test -- Dashboard ChatGptDashboardShell` |
...
| Docs-only renderer workflow updates | `./bin/docs-list`, `git diff --check`, focused Markdown link checks |
...
- Did persisted config and backend settings sync stay on the provider path?
