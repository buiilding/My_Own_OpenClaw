...
| Local tool result never returns, returns twice, or targets stale turn | SDK agent runtime, sidecar bridge, and renderer display projection | `packages/windie-sdk-js/src/runtime/WindieAgent.ts`, `packages/windie-sdk-js/src/tools/ToolExecutionCoordinator.ts`, `features/chat/hooks/chatStream/useChatStreamToolHandlers.ts` | SDK client/runtime tests, `tests/frontend/ChatStreamToolHandlers.test.ts` | [Tool Execution Lifecycle](../../tools/tool_execution_lifecycle.md), [Sidecar Tool Change Workflow](../sidecar_tool_change_workflow.md) |
