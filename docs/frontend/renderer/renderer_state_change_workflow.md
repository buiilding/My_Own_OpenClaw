...
| Local tool result never returns, returns twice, or targets stale turn | Agent SDK tool routing, local-runtime bridge, and renderer display projection | `packages/windie-sdk-js/src/runtime/Agent.ts`, `packages/windie-sdk-js/src/tools/ToolExecutionCoordinator.ts`, `features/chat/hooks/chatStream/useChatStreamToolHandlers.ts` | SDK client/runtime tests, `tests/frontend/ChatStreamToolHandlers.test.ts` | [Tool Execution Lifecycle](../../tools/tool_execution_lifecycle.md), [Sidecar Tool Change Workflow](../sidecar_tool_change_workflow.md) |
...
- Do not repair sidecar tool payloads in chat components. Fix the Agent SDK tool router, the main bridge mapper, or sidecar executor depending on where the payload first becomes wrong.
