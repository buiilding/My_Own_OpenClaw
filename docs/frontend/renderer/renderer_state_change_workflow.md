---
summary: "Workflow for changing WindieOS renderer state, chat UI, dashboard sections, settings panels, transcript projection, and provider-owned UI state without crossing backend, Electron main, or local-runtime boundaries unnecessarily."
read_when:
...
| Memory, models, settings, usage, or API-key panel state changes | Dashboard section component plus section helper | `features/dashboard/components/sections/*`, `features/dashboard/utils/*`, `app/runtime/desktopSettingsEventRuntimeClient.ts` | `tests/frontend/MemorySectionState.test.js`, `tests/frontend/SettingsSection.test.jsx`, `tests/frontend/ChatModelOptions.test.js`, `tests/frontend/DesktopSettingsEventRuntimeClient.test.ts` | [Dashboard Sections Hub](dashboard/sections/README.md), [Renderer Settings Hub](settings/README.md) |
