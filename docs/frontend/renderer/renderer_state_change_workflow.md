...
| Tool-call or tool-output cards render incorrectly after backend events | SDK current-turn projection and message payload projection | `features/chat/hooks/useConversationRuntimeProjectionStream.ts`, `features/chat/utils/state/chatBoxResponseState.js`, `features/chat/payloads/*`, `infrastructure/transcript/*` | `tests/frontend/ChatStreamThinkingStatus.state.test.tsx`, `tests/frontend/ChatBoxResponse.state.test.jsx`, `tests/frontend/ToolOutputMessageState.test.ts` | [Tool Call/Output Rendering](chat/payloads/tool_call_output_and_transparency_section_rendering_reference.md) |
| Local tool result never returns, returns twice, or targets stale turn | SDK desktop agent, sidecar bridge, and renderer display projection | `packages/windie-sdk-js/src/runtime/WindieDesktopAgent.ts`, `packages/windie-sdk-js/src/tools/ToolExecutionCoordinator.ts`, `features/chat/hooks/chatStream/useChatStreamToolHandlers.ts` | `tests/frontend/WindieSdkDesktopAgent.test.ts`, `tests/frontend/WindieSdkDesktopAgent.test.ts`, `tests/frontend/ChatStreamToolHandlers.test.ts` | [Tool Execution Lifecycle](../../tools/tool_execution_lifecycle.md), [Sidecar Tool Change Workflow](../sidecar_tool_change_workflow.md) |
