...
| Local tool result never returns, returns twice, or targets stale turn | SDK main runtime, sidecar bridge, and renderer display projection | `frontend/src/main/windie_sdk_runtime.cjs`, `frontend/src/main/ipc/ipc_sdk_tool_router.cjs`, `features/chat/hooks/chatStream/useChatStreamToolHandlers.ts` | `tests/frontend/WindieSdkMainRuntime*.test.cjs`, `tests/frontend/ChatStreamToolHandlers.test.ts` | [Retired Renderer Tool Execution Runtime](infrastructure/tool_execution_service_and_hook_runtime_reference.md), [Sidecar Tool Change Workflow](../sidecar_tool_change_workflow.md) |
...
- Do not repair sidecar tool payloads in chat components. Fix the SDK main-runtime tool router, the main bridge mapper, or sidecar executor depending on where the payload first becomes wrong.
- Do not put raw filesystem, shell, browser, or computer-use execution in renderer code. Backend tool execution belongs to the SDK main runtime and sidecar bridge; renderer code renders projections only.
