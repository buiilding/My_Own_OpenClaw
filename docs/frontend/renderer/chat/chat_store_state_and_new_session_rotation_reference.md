...
to message count, active turn ref, and last-message trace metadata before
calling the provider trace runtime; the trace runtime no longer scans raw
workspace messages itself. Chat stream and SDK current-turn
...
state, a purpose-named `viewLiveTurnRef`, and thinking source labels; it does
not carry raw messages, renderer annotations, the no-view `sdkLiveTurn`, or the
full or partial `ConversationView` payload. The current-turn projection read model keeps
...
sources when SDK sends padded or empty labels. Live-turn side-effect dedupe also
keeps SDK presentation entry ids exact, so padded ids fall back to the local
