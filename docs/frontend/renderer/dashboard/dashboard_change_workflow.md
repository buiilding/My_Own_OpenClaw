...
| Delete/clear chats leaves stale transcript, workspace, or active state | `useDashboardConversations.js`, `DashboardShell.jsx`, `DesktopConversationStoreAdapter`, `resetActiveChatSession`, workspace binding helpers | `tests/frontend/DashboardConversationLoad.test.js`, `tests/frontend/DesktopConversationStoreAdapter.test.ts`, `tests/frontend/LocalConversationStore.test.js` |
...
| Usage panel changes from placeholder to real data | `components/sections/UsageSection.jsx`, token/usage event consumers, backend token-count docs | `tests/frontend/UsageSection.test.jsx`, token-count frontend/backend tests when data becomes real |
| Dashboard layout, shell styles, responsive behavior | `frontend/src/renderer/styles/DashboardShell.css`, `CloneSettings.css`, `SettingsPanel.css`, shell components | focused frontend render/layout tests; visual/manual checks for large UI changes |
...
- `main-window-open-target` should wake dashboard layout and route only to known
  targets.
- `ipc-status` and `get-client-user-id` snapshots provide dashboard transport and
  user context. Do not duplicate backend connection state in section panels.
...
- `cd frontend && npm run test -- ConversationInferenceSessionRuntime`
- `cd frontend && npm run test -- LocalConversationStore`
