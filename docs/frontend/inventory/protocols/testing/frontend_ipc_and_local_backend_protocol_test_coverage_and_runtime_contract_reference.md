...
| outbound payload normalization | SDK `filterBackendPayload(...)` with Electron main facade `ipc_backend_payload_contract.cjs` | backend websocket contract tests and screenshot-strip tests | known command payloads are filtered to backend contract keys from the SDK-owned allowlist; client-supplied `screenshot_url` is removed from outbound payloads while keeping supported screenshot refs |
