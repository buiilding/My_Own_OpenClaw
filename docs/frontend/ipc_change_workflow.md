...
| Conversation and memory persistence | SDK local-runtime store and renderer runtime facades | transcript writer, memory panels | SDK conversation/memory store tests plus related local-runtime memory tests |
...
| Bad local-runtime payload | Inspect the SDK local-runtime caller or scoped main helper, method names, mapped keys, and sidecar handler params. | [Local Runtime JSON-RPC Change Workflow](sidecar/local_backend_jsonrpc_change_workflow.md) |
