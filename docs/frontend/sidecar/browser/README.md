---
summary: "Frontend sidecar browser docs sub-hub for the Browser Use CLI adapter, canonical browser contract validation, and Windie-owned browser launch/result boundaries."
read_when:
...
Current browser tool execution routes through `frontend/src/main/python/tools/browser/browser_use_engine.py`, which adapts WindieOS's canonical browser payloads to the official `browser-use[cli]` package. WindieOS owns schema validation, Chrome/CDP launch policy, local tool transport, browser-local files, and result normalization; Browser Use owns browser/session mechanics.
