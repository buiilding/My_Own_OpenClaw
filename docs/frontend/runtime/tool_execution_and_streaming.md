...
- screenshot prep hides whichever WindieOS surface currently owns the capture when that surface participates in the active capture-collapse path (`chatbox`, `main-window`, or none); on current runtime this means Linux overlay capture and dashboard suppression paths, while macOS/Windows overlay windows stay visible and protected instead of entering a capture-time hide/show cycle
...
- platform capture prep still routes through `platform/surfaceVisibility/*`; Linux owns the chat-pill hide/restore path with compositor settle, while Windows/macOS use a true no-op runtime and rely on always-on overlay content protection instead of active-loop-only or capture-time hide/show behavior
