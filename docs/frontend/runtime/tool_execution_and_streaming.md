...
- parses outgoing tool-result/tool-bundle-result envelopes via shared `toolRunnerBackendPayload` helpers so backend send gating uses one typed correlation contract
