...
- manual chat-pill drag position is main-owned and reused by overlay helper reposition passes so screenshot/show/hide lifecycles cannot snap the pill back to its default centered location; drag now preserves horizontal placement plus display affinity while every restore re-anchors the pill to that display's bottom work area
