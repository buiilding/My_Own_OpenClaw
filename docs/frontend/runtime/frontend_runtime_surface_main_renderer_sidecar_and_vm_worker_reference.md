...
- deferred model selection (`buildDeferredQueryModelSelection`) sent through `DesktopConversationRuntimeClient.setModel(...)` immediately before `sendQuery(...)` when needed
