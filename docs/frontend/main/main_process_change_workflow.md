...
| Local tool, memory, shell, browser, screenshot, or sidecar call fails before reaching Python | SDK local-runtime bridge and mapper | `frontend/src/main/sidecar/local_runtime*.cjs`, `frontend/src/main/sidecar/local_runtime_supervisor.cjs`, `frontend/src/main/python/local_backend.py` | `tests/frontend/LocalBackendBridge*.test.cjs`, `tests/frontend/LocalRuntimeSupervisor.test.cjs`, `tests/sidecar/test_json_rpc_protocol.py` | [Main Local-Backend Hub](local_backend/README.md), [Local Runtime Bridge Overview](local_backend_bridge_handler_and_window_guard_reference.md) |
