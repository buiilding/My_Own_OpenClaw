...
- if the screenshot sidecar returns owned `data.screenshot_path` under `${os.tmpdir()}/desktop-agent-screenshots` with a `windie-shot-` filename, bridge attempts artifact upload (`POST /api/artifacts/`)
- legacy owned `data.screenshot_path` values under `${os.tmpdir()}/windieos-screenshots` remain accepted for compatibility
