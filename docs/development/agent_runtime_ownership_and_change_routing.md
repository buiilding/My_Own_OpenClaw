...
| SDK runtime | Hosted backend websocket lifecycle, install-token identity resolution, local runtime startup/reuse, local-tool result return, normalized conversation events, conversation stores, replay/rehydrate helpers, projections reusable by Electron, CLI, plugins, and tests | Electron-only shell policy or sidecar tool implementation details |
