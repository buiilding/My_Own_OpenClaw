...
| Electron main | BrowserWindow lifecycle, IPC transport, menus, app lifecycle, native permissions, platform window policy, desktop sidecar launch option assembly, wakeword supervision, endpoint diagnostics, direct `WindieAgent.startDesktop(...)` customer wiring | Agent loop, prompt compiler, durable conversation store, websocket lifecycle, local-tool routing authority, sidecar daemon client/lifecycle ownership, or duplicate SDK runtime behavior |
