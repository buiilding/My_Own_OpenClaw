...
| backend event -> SDK/main execution | executable tool event payload | `backend/src/api/processing/formatters`, `packages/windie-sdk-js/src/tools`, `packages/windie-sdk-js/src/runtime/WindieDesktopAgent.ts` | Are model-facing args and executable args being confused? |
