...
- `debug/`: OpenClaw-style debug docs for logs, observability-change routing, diagnostic flags, endpoint/network checks, process health, trace flags, symptom playbooks, and test selection.
- `channels/`: OpenClaw-style channel docs for desktop chat, websocket transport, voice/audio workflows, sidecar tool execution, SDK clients, and VM run control routing.
...
- `backend/`: backend functionality maps and subsystem docs, including API-route, backend-service, prompt-context, query-lifecycle, and tool-turn workflow guides that route route, websocket, service, prompt, agent-loop, session, formatter, history, and tool orchestration changes to exact owners.
- `frontend/`: frontend/electron/renderer/sidecar functionality maps plus renderer-state, main-process, sidecar-runtime, IPC, and sidecar-tool change workflows for renderer/preload/main/sidecar boundaries.
