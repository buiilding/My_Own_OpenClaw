...
- `backend/`: backend functionality maps and subsystem docs, including API-route, query-lifecycle, and tool-turn workflow guides that route route, websocket, agent-loop, session, formatter, history, and tool orchestration changes to exact owners.
