...
- `install/`, `cli/`, `platforms/`, `help/`, and `web/`: broad operational entrypoints that mirror OpenClaw's public-domain navigation while staying grounded in WindieOS desktop/runtime surfaces; install now covers path selection, endpoint setup, packaged reinstall/reset, and install troubleshooting, while platforms now cover permission, screenshot/overlay, window/input, and packaging runtime matrices.
