...
- `frontend/`: frontend/electron/renderer/sidecar functionality maps plus dashboard, query-send/stream-relay, workspace-context, overlay-phase/surface, renderer-state, main-process, sidecar-runtime, local-backend JSON-RPC, IPC, and sidecar-tool change workflows for renderer/preload/main/sidecar boundaries.
