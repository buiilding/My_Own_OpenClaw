...
| Browser action launches wrong browser, loses session, or cannot attach | Sidecar browser runtime and Electron session UI | `frontend/src/main/python/tools/browser`, `frontend/src/main`, `frontend/src/renderer/features/chat` | Sidecar browser tests and frontend browser-session tests | [Browser Change Workflow](../browser/browser_change_workflow.md), [Dedicated Browser Runtime](../browser/dedicated_browser_runtime.md), [Browser Action Surface](../browser/browser_action_surface.md), [Browser Troubleshooting](../browser/browser_troubleshooting.md) |
| Screenshot, OCR, or coordinate grounding is wrong | Backend OCR/vision services plus sidecar screenshot/input | `backend/src/services/ocr`, `backend/src/services/vision`, `backend/src/core/inference`, `backend/src/tools/computer`, `frontend/src/main/python/tools/computer` | Backend OCR/vision service tests, sidecar computer tests, tool schema tests | [Backend Service Change Workflow](../backend/services/backend_service_change_workflow.md), [Computer Tools](../tools/computer.md), [OCR and Vision](../sdk/ocr_and_vision.md), [Inference Providers](../providers/inference.md) |
...
| Hosted URL, websocket auth, CORS, install token, or health check fails | Backend gateway/API and Electron endpoint forwarding | `backend/src/api`, `backend/src/api/auth`, `frontend/src/main`, `frontend/src/renderer/infrastructure/api` | Backend route/auth tests plus frontend endpoint tests | [Credential and Token Change Workflow](../security/credential_token_change_workflow.md), [API Route Change Workflow](../backend/api/api_route_change_workflow.md), [Main Process Change Workflow](../frontend/main/main_process_change_workflow.md), [Gateway Hub](../gateway/README.md), [Hosted API and Auth](../web/hosted_api_and_auth.md), [Endpoint and Network Debugging](../debug/endpoint_and_network_debugging.md) |
| Websocket event is missing, malformed, ignored, stale, or updates the wrong UI state | Backend event/formatter contract plus renderer event consumers | `backend/src/core/events`, `backend/src/api/contracts`, `backend/src/api/processing/formatters`, `backend/src/api/schemas/outgoing.py`, `frontend/src/renderer/types/backendEvents.ts`, `frontend/src/renderer/features/chat/utils/chatStream` | Backend event/formatter/schema tests plus frontend chat-stream event tests | [WebSocket Event Contract Change Workflow](../channels/websocket_event_contract_change_workflow.md), [WebSocket Event Reference](websocket_event_reference.md), [Frontend Stream State Machine](../frontend/runtime/stream_event_state_machine.md) |
...
| LLM provider, model catalog, credential, or failover behavior changes | Backend LLM stack plus frontend settings surface | `backend/src/llm/providers`, `backend/src/llm/models`, `backend/src/core/config`, `frontend/src/renderer/features/settings` | Backend provider/config tests plus frontend model-settings tests | [Credential and Token Change Workflow](../security/credential_token_change_workflow.md), [Providers Hub](../providers/README.md), [Models and LLM Providers](../providers/models.md), [Provider Credentials](../providers/credentials.md), [Backend Config and Container Change Workflow](../backend/config/backend_config_and_container_change_workflow.md) |
| Backend config, runtime normalization, DI rebinding, or settings propagation changes | Backend config/container/session runtime | `backend/src/core/config`, `backend/src/core/container`, `backend/src/agent/session`, `backend/src/core/validation` | Backend config, container, settings, provider, and session tests | [Backend Config and Container Change Workflow](../backend/config/backend_config_and_container_change_workflow.md), [Configuration Reference](configuration_reference.md), [Container DI and Initialization Lifecycle Reference](../backend/bootstrap/container_di_and_init_lifecycle_reference.md) |
| Prompt text, repo instructions, model-visible tool schemas, or prompt transparency changes | Backend prompt constructor, tool policy, and event presenter | `backend/src/llm/prompts`, `backend/src/agent/llm`, `backend/src/tools`, `prompts/schema.txt` | Prompt constructor/manager tests, schema/policy tests, transparency formatter tests | [Prompt Context Change Workflow](../backend/llm/prompts/prompt_context_change_workflow.md), [Prompt and Tool Context](../concepts/prompt_and_tool_context.md), [Tool Policy Profiles and Capabilities](../tools/tool_policy_profiles_and_capabilities.md) |
...
| Wrong chat, stale stream, session identity, conversation resume, or rehydrate routing changes | Renderer transcript/session runtime plus backend session registry and sidecar transcript storage | `frontend/src/renderer/infrastructure/transcript`, `frontend/src/main/ipc/ipc_transcript_session_sync.cjs`, `backend/src/agent/session`, `backend/src/api/services/rehydrate_*`, `frontend/src/main/python/memory` | Frontend transcript/session/replay tests, backend session/rehydrate tests, sidecar conversation tests | [Session and Conversation Identity Change Workflow](../memory/session_conversation_identity_change_workflow.md), [Session and Transcript Reference](session_and_transcript_reference.md), [Memory Change Workflow](../memory/memory_change_workflow.md) |
...
| `backend/src/api/auth` | Install-token auth, websocket auth, runs API auth, hosted access checks | [Credential and Token Change Workflow](../security/credential_token_change_workflow.md), [Hosted Backend Auth](../operations/hosted_backend_auth.md), [Security Boundary Matrix](../security/security_boundary_matrix.md) |
...
| `backend/src/core/config` | App config, env var loading, credential access | [Backend Config and Container Change Workflow](../backend/config/backend_config_and_container_change_workflow.md), [Configuration Reference](configuration_reference.md), [Provider Credentials](../providers/credentials.md) |
