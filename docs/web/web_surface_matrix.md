...
WindieOS is desktop-first. Web-facing surfaces are either the public landing page or hosted backend APIs used by the desktop app, SDK clients, and VM workers. Do not assume the Electron renderer dashboard is safe to host in a normal browser without checking Electron IPC and local sidecar assumptions.
...
| --- | --- | --- |
| update public landing copy/sections | [Landing Page](landing_page.md) | `frontend/src/landing/*` |
...
- Local desktop tools remain sidecar-executed; do not expose local filesystem/shell/computer control as generic hosted web endpoints.
