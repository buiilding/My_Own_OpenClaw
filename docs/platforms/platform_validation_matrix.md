...
read_when:
  ...
  - When validating a platform fix that touches Electron main, renderer overlays, sidecar computer tools, permissions, or packaging scripts.
...
| screenshot visibility bridge | `tests/frontend/LocalBackendBridgeWindowVisibility.test.cjs`, `tests/frontend/SurfaceOrchestratorSurfaceVisibility.test.ts` | `tests/sidecar/test_screenshot_tool.py` | Use when screenshots include WindieOS surfaces or capture bounds are wrong. |
