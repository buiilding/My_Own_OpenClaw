...
| Tool display projection | `frontend/src/renderer/features/chat/hooks/chatStream/useChatStreamToolHandlers.ts`, `frontend/src/renderer/app/runtime/{desktopCurrentTurnMessageRuntime.js,desktopSdkDisplayChatMessageProjectionRuntime.ts}`, `frontend/src/renderer/infrastructure/transcript/toolCallMessageState.js` | Renders tool-call/tool-output cards from SDK/main fan-out without executing local tools in the renderer. |
