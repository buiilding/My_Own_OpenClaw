...
Replay SDK command handlers for `conversation.editAndResend` and
`conversation.retryTurn` remain intent-only at this boundary. Main forwards
only exact `userId`, `conversationRef`, target `messageId`, and edit text for
edit/resend; stale caller-supplied `payload`, `model`, and `turnRef` fields are
ignored instead of being converted into replay runtime context. Retry requires
an exact target `messageId`.
...
| `desktopRendererDisplayProjectionDiagnosticsClient.ts` | Removed unused diagnostics client | This file had no production, test, or docs consumers and duplicated the `renderer.display_projection` diagnostics path already sanitized by Electron main diagnostics storage. | Do not reintroduce. Renderer display projection diagnostics should go through an active runtime owner with a real caller, then through `diagnostics.append` and the main diagnostics sanitizer. |
...
The renderer live-turn facade accepts only query text, exact conversation/turn
