...
| Response overlay and dashboard thread presentation render materialized active-turn `ConversationView.displayRows` before non-materialized live entries, so assistant text and tool call/output cards remain visible through one row owner after the SDK removes the same materialized live entries from `liveTurn.entries`; renderer presentation also suppresses stale mixed-view live text when a display row already covers it to prevent transient duplicate assistant bubbles. | `ResponseOverlayViewContract.test.ts`, `MessagePresentationPipeline.test.js`, `AgentSdkConversationRuntime.test.ts` |
...
| Retry/edit resend keeps the accepted child display revision visible if the later normal send fails, clearing only the pending turn and appending a send-failure row instead of rolling back to the parent transcript; renderer replay dispatch can use exact SDK row `conversationRef` action metadata when transcript scope is unavailable, without authorizing replay from chat-store active-conversation fallback. | `ConversationReplayActions.test.jsx`, `DesktopConversationReplayRuntime.test.js`, `DesktopMessageActionRuntime.test.js`, `SdkDisplayChatMessageProjection.test.ts`, `MessageListAssistantActions.test.jsx` |
...
| Tool-call rows pair with tool-output rows after replay. | `ToolCallMessageState.test.js`, `DesktopCurrentTurnMessageRuntime.test.js`, `SdkDisplayChatMessageProjection.test.ts`, `ConversationRuntimeProjectionStream.test.ts` |
