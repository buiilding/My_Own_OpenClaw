...
| Local runtime implementation | local JSON-RPC, computer/filesystem/system/browser tools, local memory, wakeword services, backend HTTP clients | [Local Runtime Python Implementation Docs Hub](../frontend/sidecar/README.md) | `<windie> test local-runtime` or focused `./scripts/python-in-env local-runtime python -m pytest frontend/tests/sidecar/...` |
