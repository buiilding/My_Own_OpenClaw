...
| Local-runtime SQLite/memory schema | `./scripts/python-in-env local-runtime python -m pytest frontend/tests/sidecar/test_local_store_init.py frontend/tests/sidecar/test_local_backend.py frontend/tests/sidecar/test_memory_operations.py` |
| Local-runtime FAISS/vector mapping | local-runtime memory delete/search/init tests and corrupted-index coverage |
| Semanticization/watermark | `./scripts/python-in-env local-runtime python -m pytest frontend/tests/sidecar/test_memory_summarizer.py` plus semanticization tests |
