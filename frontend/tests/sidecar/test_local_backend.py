...
def test_resolve_local_runtime_log_level_defaults_to_warning(monkeypatch):
    ...
    monkeypatch.delenv(
        local_backend_module.ENV_WINDIE_LOCAL_RUNTIME_LOG_LEVEL, raising=False
    )
...
def test_resolve_local_runtime_log_level_accepts_valid_levels(monkeypatch):
    monkeypatch.setenv(
        local_backend_module.ENV_WINDIE_LOCAL_RUNTIME_LOG_LEVEL, "info"
    )
...
def test_resolve_local_runtime_log_level_prefers_generic_local_runtime_env(monkeypatch):
    monkeypatch.setenv(
        local_backend_module.ENV_WINDIE_LOCAL_RUNTIME_LOG_LEVEL, "debug"
    )
    monkeypatch.setenv(
...
def test_resolve_local_runtime_log_level_falls_back_on_invalid_value(monkeypatch):
    monkeypatch.setenv(
        local_backend_module.ENV_WINDIE_LOCAL_RUNTIME_LOG_LEVEL, "verbose-ish"
    )
...
def test_local_runtime_log_level_helper_uses_runtime_boundary_name():
    ...
    retired_helper_name = "_resolve_" + "sidecar" + "_log_level"
    assert retired_helper_name not in source
    assert "AGENT_" + "SIDE" + "CAR_LOG_LEVEL" not in source
    assert "WINDIE_" + "SIDE" + "CAR_LOG_LEVEL" not in source
