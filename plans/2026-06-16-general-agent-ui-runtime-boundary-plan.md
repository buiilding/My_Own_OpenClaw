...
- Finding: `AgentRegisterMcpOptions` only renamed the `{ replace?: boolean }`
  options object accepted by `Agent.registerMcps`, and exact scans found no
  callers outside the defining module.
- Change: removed the exported type alias, inlined the options shape on
  `registerMcps`, and added package-boundary coverage so the alias stays absent.
- Validation: focused WindieSdkPackageBoundary Jest test, SDK TypeScript
  `--noEmit` compile, Agent.registerMcps docs route lookup, and stale alias scan.
- Compatibility: no repo migration required because there were no internal
  callers. External SDK callers should type MCP registration options directly as
  `{ replace?: boolean }` when needed.
...
- Finding: SDK-generated untouched agent definitions still emitted the
  backend/product-specific `windie_default` mode, leaking WindieOS vocabulary
  through the generic SDK and Electron host contract.
- Change: the TypeScript SDK builder now emits the generic `default` mode,
  the backend schema normalizes legacy `windie_default` input to `default`,
  SDK docs describe the canonical mode, focused frontend/backend tests cover
  the contract, and generated CJS AgentDefinition output was regenerated.
- Validation: `npm.cmd run build:cjs` in `packages/windie-sdk-js`, full
  `WindieSdkClient.test.ts`, `WindieSdkPackageBoundary.test.ts`,
  `WindieSdkPrivateExports.test.cjs`, focused backend `test_common_schemas.py`,
  docs listing, stale literal scan, and diff checks.
- Compatibility: no migration required. Existing clients that still send
  `windie_default` remain accepted at the backend validation boundary and are
  normalized before downstream use; new SDK-authored payloads use `default`.
