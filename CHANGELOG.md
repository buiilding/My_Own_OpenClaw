...
- Added generic SDK createAgentLocalRuntimeProvider factory alias while keeping createWindieLocalRuntimeProvider compatibility.
...
- Added generic SDK AgentModelSelection type alias while keeping WindieModelSelection compatibility.

- Added generic SDK gentBuiltins helper aliases while keeping windieBuiltins compatibility.

- Genericized the Electron main local-runtime bridge dependency names while preserving legacy sidecar bridge exports.
...
- Genericized generated Python SDK agent ids, names, and default conversation refs.

- Moved the default browser-owned file root to the generic desktop-agent directory.
...
- Genericized the sidecar daemon health service label while preserving daemon routes and runtime behavior.

- Genericized sidecar plugin entrypoint import module names while preserving plugin manifest contracts.
...
- Renderer: use a generic agent model-selection type alias in desktop runtime facades.

- Tests: align SDK command helper mock names with the generic agent SDK command path.

- Main: rename the internal SDK command handler module to the generic agent SDK host name.

- Tests: remove retired main agent-host wrapper tombstone probes from SDK boundary coverage.
...
- Main: remove dormant context-label window ownership from the generic overlay surface runtime and docs.

- Renderer: rename shared appearance palette CSS variables from Windie-specific names to generic agent palette names.
...
- main: Route app lifecycle shutdown through a local-runtime dependency name.

- docs: Route docs-search results through normalized runtime-owner paths on Windows.

- frontend/renderer: restore fail-safe SDK conversation-event ingress dispatch and rename stale backend-ingress docs.

- docs/frontend: describe renderer voice transcription through the desktop voice runtime gateway facade.

- frontend/main: rename host skin local-runtime copy keys away from local-backend terminology.

- frontend/main: rename local runtime readiness locals away from backend-prefixed wording while preserving compatibility payloads.
...
- frontend/main: rename the local runtime readiness helper away from backend-ready terminology.
...
- backend/core: remove the unused core container package export so the
  runtime container facade is imported from its owner module.
...
- frontend/sidecar: describe Python sidecar runtime docstrings and lifecycle logs as local sidecar runtime behavior while preserving LocalBackend compatibility names.

- tests/frontend: align dashboard startup retry coverage with local-runtime readiness wording while keeping legacy local-backend compatibility covered in the facade tests.
