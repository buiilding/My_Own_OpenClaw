...
- fix(frontend-stop-loop): make Stop immediately mark the active turn complete in dashboard/chat-pill state and ignore stale/terminated-turn `tool-call`/`tool-bundle` events in `useToolRunner`, so the agent loop does not continue dispatching new tool executions after Stop
