...
### Fixed
- fix(agent-final-response): enforce terminal query events (always emit `streaming-complete`/`error`) to prevent indefinite frontend waiting, add empty-final-response fallback messaging, and broaden Anthropic/Kimi completion text extraction (`choice.text`, `content[].content`, `output_text`)
