...
### Added
- fix(backend-artifact-store-persistence): move default `artifact_store_path` from temp dir to persistent user config storage (`DesktopAssistant/artifacts`) so historical `screenshot_ref` attachments survive temp cleanup/restarts; add backend config-model regressions for Windows/Linux default path resolution and update config docs.
