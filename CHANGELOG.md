...
- feat(mouse-ocr-only-model-contract): enforce OCR-only `mouse_control.find_coordinates_by` for model-facing schemas and parser validation (manual/prediction removed from model contract), while keeping manual coordinate handling as an internal execution path after backend grounding rewrite.
