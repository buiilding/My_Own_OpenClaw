...
### Fixed
- fix(frontend-chat): keep main dashboard visible when `include_query_screenshot` is disabled by routing window-switch behavior through explicit `returnToChatboxPolicy` (`never|auto|always`) and defaulting dashboard send flow to `auto`, so no-image mode no longer hides the main window
