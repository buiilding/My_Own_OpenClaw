...
### Added
- refactor(backend-browser-schemas): split OpenClaw compatibility schema into `backend/src/tools/browser/openclaw_compat_schema.py` and import shared compat action types into `backend/src/tools/browser/schemas.py` to reduce oversized module growth; add compatibility coverage in `tests/backend/test_browser_remote_tool.py`
