...
- test(ghost-overlay-debug): add dedicated renderer ghost debug view (`/?view=tool-ghost-debug`) and launcher `npm run test:ghost-overlay` for visual top-to-bottom ghost-only animation (no real cursor movement/click); add `ToolGhostDebugApp` timeline regression test
- feat(frontend-main-overlay-debug): add OS-level ghost overlay debug mode (`WINDIE_DEBUG_GHOST_OVERLAY=1`, `npm run electron:ghost-overlay`) so Electron response window renders `tool-ghost-debug` directly on desktop and ignores backend phase auto-hide during visual verification
- test(ghost-screen-click): replace browser visual harness with hard-coded real screen test script (`scripts/test-ghost-screen-click.py`) and frontend launcher `npm run test:ghost-screen` for manual 1920x1080 top-to-bottom cursor movement verification (no click by default; `--with-click` optional, `--dry-run` supported)
