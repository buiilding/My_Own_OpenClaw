...
- test(ghost-cursor): add single WindieOS ghost-cursor animation test launcher `npm run test:ghost-cursor` (OS overlay in Electron, no real cursor movement/click) backed by minimal `tool-ghost-debug` mode that renders only one moving ghost cursor and timeline regression coverage
