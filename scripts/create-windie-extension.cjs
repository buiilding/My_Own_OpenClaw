...
function usage() {
  return [
    'Usage: bin/windie extension create <plugin-id> [options]',
    '',
    'Options:',
...
function buildFiles({ extensionId, name, toolName }) {
  ...
  return {
    ...
    ['plugin.json', JSON.stringify({
      id: extensionId,
      name: displayName,
      description: `Starter Windie sidecar plugin for ${displayName}.`,
      tools: [
        {
          name: toolName,
    ...
    ['README.md', [
      `# ${displayName} Plugin`,
      '',
      'Starter Windie sidecar plugin generated by `bin/windie extension create`.',
      '',
      '## Contents',
      '',
      `- \`plugin.json\` declares the sidecar plugin and the \`${toolName}\` tool.`,
    ...
    ['README.md', [
      `# ${displayName} Skill`,
      '',
      'Starter prompt skill paired with the sidecar plugin.',
      '',
    ].join('\n')],
  ]),
  };
...
function runCli(argv = process.argv.slice(2)) {
  ...
  process.stdout.write(`Created Windie plugin at ${result.pluginDir}\n`);
  process.stdout.write(`Created Windie skill at ${result.skillDir}\n`);
