...
from backend.src.tools.registry import ToolRegistry, ToolMetadata
...
class ToolSearchEngine:
    ...
    def index_tools(self) -> None:
        ...
        tools = self.tool_registry.marketplace_tools
        if not tools:
    ...
    def _simple_text_search(self, query: str, limit: int = 5) -> List[ToolSearchResult]:
        ...
        results = []
        for tool_name, metadata in self.tool_registry.marketplace_tools.items():
            searchable_text = self._create_searchable_text(metadata)
    ...
    def find_tool_by_name(self, tool_name: str) -> Optional[ToolSearchResult]:
        ...
        """
        metadata = self.tool_registry.marketplace_tools.get(tool_name)
        if metadata:
