...
class ToolExecutionAudit:
    ...
    def _summarize_size(self, value: Any) -> str:
        try:
            if isinstance(value, str):
                return f"size={len(value)} chars"
            if isinstance(value, (bytes, bytearray, memoryview)):
                return f"size={len(value)} bytes"
            if isinstance(value, (dict, list, tuple, set, frozenset)):
                return f"size={len(value)} items"
        except Exception:
            pass
        return "size=unknown"

    def _excluded_summary(self, value: Any) -> str:
        return f"[EXCLUDED: {type(value).__name__}, {self._summarize_size(value)}]"
    ...
    def _sanitize_sequence(
        ...
        if value_id in visited:
            return [
                "[CYCLE: Circular reference detected - skipping to prevent infinite recursion]"
            ]
        visited.add(value_id)
        try:
            sanitized_list = [
                self._sanitize_value(item, visited, depth + 1)
                for item in islice(value, self.MAX_SEQUENCE_ITEMS)
            ]
    ...
    def _sanitize_value(self, value: Any, visited: Set[int], depth: int) -> Any:
        ...
        if isinstance(value, str):
            if len(value) > self.MAX_PARAM_VALUE_SIZE:
                return (
                    value[: self.MAX_PARAM_VALUE_SIZE]
                    + f"... [TRUNCATED: {len(value)} chars]"
                )
            return value

        if isinstance(value, (bytes, bytearray, memoryview)):
            return f"[BYTES: {type(value).__name__}, {self._summarize_size(value)}]"

        if isinstance(value, dict):
            return self._sanitize_parameters(value, visited, depth + 1)

        if isinstance(value, (list, tuple, set, frozenset)):
            return self._sanitize_sequence(value, visited, depth)

        return f"[OBJECT: {type(value).__name__}]"

    def _sanitize_parameters(
        self, params: Dict[str, Any], visited: Optional[Set[int]] = None, depth: int = 0
    ) -> Dict[str, Any]:
        """
        ...
        if depth > self.MAX_RECURSION_DEPTH:
            return {
                "[ERROR]": "Maximum recursion depth exceeded - structure too deeply nested"
            }
        ...
        try:
            for key, value in params.items():
                ...
                if isinstance(key, str) and key.lower() in self.EXCLUDED_PARAM_KEYS:
                    sanitized[key] = self._excluded_summary(value)
                    continue

                sanitized[key] = self._sanitize_value(value, visited, depth)
        finally:
...
class SecurityPolicy:
    ...
    def grant_permission(self, tool_name: str, permission: Permission) -> None:
        """Grant one permission to a tool after declaration/policy review."""
        self.granted_permissions.setdefault(tool_name, set()).add(permission)
    ...
    def grant_permissions(
        ...
        """Grant multiple permissions to a tool after declaration/policy review."""
        self.granted_permissions.setdefault(tool_name, set()).update(permissions)

    def revoke_permission(self, tool_name: str, permission: Permission) -> None:
        """Revoke one previously granted permission from a tool."""
        permissions = self.granted_permissions.get(tool_name)
        if not permissions:
            return
        permissions.discard(permission)
        if not permissions:
            self.granted_permissions.pop(tool_name, None)
    ...
    def check_permission(
        ...
        granted = self.granted_permissions.get(tool_name, set())
        if permission not in granted:
            logger.warning(
                f"Security Violation: Tool '{tool_name}' has declared action "
                f"'{permission}' but it is not in granted_permissions. Action DENIED."
            )
            return False
    ...
    def check_resource_limits(
        self, tool_name: str, estimated_time: Optional[float] = None
    ) -> bool:
        """
