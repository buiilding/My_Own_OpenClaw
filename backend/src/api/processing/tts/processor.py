...
class TTSProcessor:
    ...
    async def process_event(
        self, event: AgentStreamingEvent, tts_service: Optional[SpeechService]
    ) -> None:
        """
    ...
    async def _process_chunk(
        self, chunk: ChunkEvent, tts_service: SpeechService
    ) -> None:
        """
                ...
                if exit_idx != -1:
                    ...
                    if post_exit:
                        await self._process_chunk(
                            ChunkEvent(content=post_exit), tts_service
                        )
                    return
                else:
                ...
                for i, char in enumerate(content):
                    if char == "{":
                        ...
                        if self._json_brace_depth == 0:
                            ...
                            logger.debug(
                                "TTS: JSON exit detected (brace depth 0), resuming audio output"
                            )
        ...
        if self._is_tool_call_context is None:
            ...
            if len(self._stream_buffer) > MAX_BUFFER_SIZE:
                ...
                await self.tts_manager.process_event(
                    tts_service, ChunkEvent(content=self._stream_buffer)
                )
                self._stream_buffer = ""
                return
        ...
        # 3. IF TEXT MODE (Pass-through with scanning)
            ...
            if marker_idx != -1:
                ...
                if pre_marker:
                    await self.tts_manager.process_event(
                        tts_service, ChunkEvent(content=pre_marker)
                    )
                ...
                if marker == code_marker:
                    self._suppression_type = "code"
                    logger.debug(
                        "TTS: Code block entry detected mid-chunk, entering suppression"
                    )
                else:
                    self._suppression_type = "json"
                    self._json_brace_depth = 1  # We found the opening brace
                    logger.debug(
                        "TTS: JSON entry detected mid-chunk, entering suppression"
                    )
