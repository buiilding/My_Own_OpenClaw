...
class RehydrateEntryNormalizer:
    ...
    def extract_tool_call_details(
        ...
        *,
        ...
        parsed_name = self.normalize_optional_string(
            payload.get("name")
        ...
        parsed_call_id = self.normalize_optional_string(
            payload.get("id")
            if function_payload is None
        ...
        thought_signature = self.extract_thought_signature(
            payload,
            function_payload,
        )
    ...
    def normalize_tool_calls(self, raw_tool_calls: Any) -> List[Dict[str, Any]]:
        ...
        for index, raw_call in enumerate(raw_tool_calls):
            ...
            thought_signature: Optional[str] = self.extract_thought_signature(raw_call)
            ...
            if isinstance(raw_call.get("name"), str) and raw_call.get("name", "").strip():
                ...
                if thought_signature is None:
                    thought_signature = self.extract_thought_signature(function_block)
