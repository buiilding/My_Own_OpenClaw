...
class ComputerUsePlugin(AgentPlugin):
    ...
    async def initialize(self, container: Any = None) -> None:
        ...
        if container:
            ...
            if hasattr(container, 'config') and hasattr(container.config, 'screenshot_delay_after_action'):
                 self.screenshot_delay = container.config.screenshot_delay_after_action
            logger.info("ComputerUsePlugin initialized with ToolRegistry from container")
        else:
            logger.warning("ComputerUsePlugin initialized without container, some features may fail")
    ...
    async def on_tool_end(self, tool_name: str, result: Any) -> Optional[PluginResult]:
        ...
        if not self.tool_registry:
            logger.warning("ToolRegistry not available in ComputerUsePlugin")
            return None
        ...
        # Execute screenshot tool using registry's execute_tool method
        try:
            screenshot_result = await self.tool_registry.execute_tool(SCREENSHOT_TOOL_NAME, {})
            ...
            logger.error(f"Error capturing screenshot after {tool_name}: {e}", exc_info=True)
            return None
    ...
    def _extract_screenshot_data(self, result: Any) -> Optional[str]:
        ...
        if not isinstance(screenshot_data, str):
            logger.warning(f"Invalid screenshot data type: {type(screenshot_data)}, expected str")
            return None
