...
def _selected_model_id(session: "AgentSession") -> str | None:
...
class ToolResultProcessor:
    ...
    async def process(
        ...
        """
        model_id = _selected_model_id(session)
        ...
        try:
            for result in orchestration_result.tool_results:
                ...
                processed = await self.result_transformer.transform(
                    result.tool_call.tool_name, result.result, model_id=model_id
                )
            ...
            cleaned_count = session.get_result_storage().cleanup_request_ids(
                all_request_ids
            )
            if cleaned_count > 0:
                logger.debug(
                    f"Cleaned up {cleaned_count} tool results after processing"
                )
