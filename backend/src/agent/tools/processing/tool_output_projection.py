...
def _truncate_for_tokens(text: str, token_limit: int) -> tuple[str, int, bool]:
    ...
    if char_limit <= len(marker) + 2:
        return text[:char_limit], original_tokens, True

    available = char_limit - len(marker)
    head = max(available // 2, 1)
    tail = max(available - head, 1)
    return f"{text[:head]}{marker}{text[-tail:]}", original_tokens, True
...
def canonicalize_tool_result_for_model(
    ...
    *,
    ...
    model_content, original_tokens, truncated = _truncate_for_tokens(
