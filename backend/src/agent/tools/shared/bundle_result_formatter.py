...
def _xml_text(value: Any, default: str = "Unknown") -> str:
    if value is None:
...
class BundleResultFormatter:
    ...
    @staticmethod
    def format(
        bundle_result: Dict[str, Any], system_state: Optional[Dict[str, Any]] = None
    ) -> str:
        """
...
def _format_system_state_xml(system_state: Dict[str, Any]) -> str:
    ...
    """
    active_window = _xml_text(system_state.get("active_window", "Unknown"))
    mouse_position = system_state.get("mouse_position", {})
    mouse_x = _xml_text(
        mouse_position.get("x", 0) if isinstance(mouse_position, dict) else 0, "0"
    )
    mouse_y = _xml_text(
        mouse_position.get("y", 0) if isinstance(mouse_position, dict) else 0, "0"
    )
    time_str = _xml_text(system_state.get("time", "Unknown"))
