...
class ToolResultStorage:
    ...
    @staticmethod
    def _cancel_unresolved_future(future: asyncio.Future) -> None:
        """Wake waiters when storage cleanup removes an unresolved future."""
        if not future.done():
            future.cancel()
    ...
    def remove_pending_result(self, request_id: str) -> bool:
        ...
        if result is not None:
            ...
            logger.debug(
                f"Removed pending tool result for request_id {request_id[:15]}"
            )
            return True
        return False
    ...
    def create_result_future(self, request_id: str) -> asyncio.Future:
        ...
        """
        existing_future = self._result_futures.get(request_id)
        if existing_future is not None and not existing_future.done():
            logger.debug(
                f"Reusing result future for duplicate request_id {request_id[:15]}"
            )
    ...
    def remove_result_future(self, request_id: str) -> bool:
        ...
        if future is not None:
            self._cancel_unresolved_future(future)
            if request_id not in self._pending_results:
                self._result_timestamps.pop(request_id, None)
    ...
    def remove_bundled_result(self, bundle_request_id: str) -> bool:
        ...
        if result is not None:
            ...
            logger.debug(
                f"Removed bundled result for bundle_id {bundle_request_id[:15]}"
            )
            return True
        return False
    ...
    def create_bundle_future(self, bundle_id: str) -> asyncio.Future:
        ...
        """
        existing_future = self._bundle_futures.get(bundle_id)
        if existing_future is not None and not existing_future.done():
            logger.debug(
                f"Reusing bundle future for duplicate bundle_id {bundle_id[:15]}"
            )
            return existing_future
        if existing_future is not None:
    ...
    def cleanup_old_results(self, max_age_seconds: Optional[int] = None) -> int:
        ...
        expired_request_ids = [
            req_id
            for req_id, timestamp in self._result_timestamps.items()
        ...
        expired_bundle_ids = [
            bundle_id
            for bundle_id, timestamp in self._bundle_timestamps.items()
            if current_time - timestamp > max_age_seconds
        ]
        ...
        if cleaned_count > 0:
            logger.info(
                f"Cleaned up {cleaned_count} old tool results (TTL: {max_age_seconds}s)"
            )
    ...
    def clear_all(self) -> None:
        ...
        for future in self._result_futures.values():
            self._cancel_unresolved_future(future)
        for future in self._bundle_futures.values():
            self._cancel_unresolved_future(future)
