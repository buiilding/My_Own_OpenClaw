...
class KimiCodeProvider(LLMProvider):
    """Provider for Kimi Code (OpenAI-compatible) models."""
    ...
    def _validate_dependencies(self) -> None:
        """Kimi Code requires an API key."""
        if not self.api_key:
            raise ValueError("KimiCodeProvider requires an 'api_key'.")
    ...
    async def get_completion(
        ...
        try:
            ...
            if (
                ...
                raise LLMAPIError("Invalid response from Kimi Code", model=model)
            content = response.choices[0].message.content or ""
            return {"content": content}
        except litellm_exceptions.RateLimitError as e:
            raise LLMRateLimitError("Kimi Code rate limit exceeded", model=model, cause=e)
        except litellm_exceptions.APIError as e:
            raise LLMAPIError("Kimi Code API error", model=model, cause=e)
        except Exception as e:
            raise LLMError("An unexpected error occurred with Kimi Code", model=model, cause=e)
    ...
    async def list_models(self) -> List[Dict[str, str]]:
        """Lists available Kimi Code models (static config preferred)."""
        return []
