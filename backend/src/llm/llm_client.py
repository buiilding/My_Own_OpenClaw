...
class LiteLLMClient(LLMClient):
    ...
    async def get_completion(
        ...
        try:
            ...
            if not response:
                raise LLMAPIError("LLM returned None response", model=model)
            if (
                not hasattr(response, "choices")
                or not isinstance(response.choices, list)
                or len(response.choices) == 0
            ):
                raise LLMAPIError("LLM response missing or empty choices list", model=model)
            if not response.choices[0]:
                raise LLMAPIError("LLM response choices[0] is None", model=model)
            if (
                not hasattr(response.choices[0], "message")
                or not response.choices[0].message
            ):
                raise LLMAPIError("LLM response choices[0] missing message attribute", model=model)
            ...
            raise LLMRateLimitError(f"LLM rate limit exceeded: {e}", model=model, cause=e) from e
        except litellm_exceptions.APIError as e:
            raise LLMAPIError(f"LLM API error: {e}", model=model, cause=e) from e
        except Exception as e:
            raise LLMError(f"An unexpected LLM error occurred: {e}", model=model, cause=e) from e
    ...
    async def get_completion_stream(
        ...
        try:
            ...
            raise LLMRateLimitError(
                f"LLM rate limit exceeded: {e}. Please wait a moment and try again.",
                model=model,
                cause=e
            ) from e
        except litellm_exceptions.APIError as e:
...
def get_llm_client(cfg: AppConfig) -> LLMClient:
    ...
    Uses caching to reuse clients with the same configuration.
    ...
    # Create hash from relevant config fields
    config_dict = {
        "model_mode": cfg.model_mode,
        "model_provider": cfg.model_provider,
        "selected_model_id": cfg.selected_model_id,
        "llm_timeout": cfg.llm_timeout,
        "debug_litellm": cfg.debug_litellm,
        "api_key": cfg.api_key,  # Include API key in hash
    }
    
    # Add base_url if available
    try:
        if cfg.model_mode == "local" and cfg.model_provider:
            provider_config = cfg.llm_providers.get_provider_config(cfg.model_provider)
            config_dict["base_url"] = getattr(provider_config, "base_url", None)
    except ValueError:
        pass
    ...
    # Try cache first
    cache_key = cache_manager.get_llm_client_key(config_hash)
    cached_client = cache_manager.llm_clients.get(cache_key)
    
    if cached_client is not None:
        logger.debug(f"Using cached LLM client for config hash: {config_hash[:8]}")
    ...
    # Cache the client (long TTL since it's tied to config hash)
    cache_manager.llm_clients.set(cache_key, client, ttl=86400.0)  # 24 hours
